...
// ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = Worker 는 로컬 디스크가 없으므로 같은 순번 규칙(내용 같으면 덮어쓰기 · 다르면 _N)을 R2 에 있는 파일과 비교해 적용 = raw 덮어쓰기 손실 0 (정본 §)
export async function versionedNameR2(
  prefixDir: string,
  stemFile: string,
  newContentHash: string,
  hashOfBody: (body: string) => string | null,
): Promise<string> {
  const base = stemFile.replace(/\.json$/, "");
  const objs = await listR2(`${prefixDir}/${base}`);
  let maxN = -1;
  for (const o of objs) {
    const name = o.key.slice(prefixDir.length + 1);
    const p = parseRawName(name);
    if (!p || p.base !== base) continue;
    const buf = await getFromR2(o.key);
    const h = buf ? hashOfBody(buf.toString("utf8")) : null;
    if (h !== null && h === newContentHash) return name;
    if (p.n > maxN) maxN = p.n;
  }
  return maxN < 0 ? `${base}.json` : `${base}_${maxN + 1}.json`;
}
