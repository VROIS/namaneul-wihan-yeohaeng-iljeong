...
import { rawName, versionedNameR2, rawHash } from "./raw-filename";
...
export async function saveCollectedRaw(
  ...
  try {
    ...
    let fileName = stem;
    // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 순번 판정 기준 = R2 에 있는 같은 stem 파일 (Worker 는 디스크 없음) = 1차 raw 가 2차에 덮이는 손실 0 (정본 §)
    try {
      fileName = await versionedNameR2(
        ...
        (body: string) => {
          try {
            return rawHash(JSON.parse(body)?.[opts.hashKey]);
          } catch {
            return null;
          }
        },
