...
import {
  ...
  LANGS,
  type Lang,
} from "../shared/language-instruction";
import { recordExternalCall } from "../shared/external-call-log";
...
export async function step1_geminiItinerary(
  ...
  // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 여정 출력 언어 = 사용자 설정 언어(formData.language) 를 프롬프트에 동적 주입. ko 면 PSR 한국어 공유컬럼, 그 외 언어는 place_translations (정본 §)
  const outLang: Lang = (LANGS as readonly string[]).includes(
    formData.language || "",
  )
    ? (formData.language as Lang)
    : "ko";
  const outLangLine = `OUTPUT_LANGUAGE: ${outLang} = ${getLanguageInstruction(outLang)} (k·r·s·theme 는 OUTPUT_LANGUAGE 로 쓴다. 아래 OUTPUT 예시의 한국어 문구는 형식 예시일 뿐이다)`;
...
{"days":[{"day":1,"theme":"테마","places":[
  ...
  try {
    // ⚠️ 수정금지(승인필요) 2026-07-18 사장님 확정 = 발굴 검증표준과 완전 통일 = googleSearch 그라운딩 실제 켬 + responseMimeType 제거.
    ...
    // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = MIX step1 Gemini 호출도 관리자 대시보드 장부(external_calls)에 호출마다 1건 기록 (정본 §)
    await recordExternalCall({
      provider: "gemini",
      sku: STEP1_MODEL,
      cityId: cityId ?? null,
      tag: "90-mix-gemini_step1",
    ...
    // ⚠️ 수정금지(승인필요) 2026-09-03 사장님 결정 = 제미니가 계산한 시각(ts)·체류(m)를 그대로 받아 슬롯에 표현한다 (정본 B4 v26 세트)
