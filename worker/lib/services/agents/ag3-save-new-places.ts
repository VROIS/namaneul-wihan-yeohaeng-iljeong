...
// ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 다른 언어 문구 저장 = place_translations 갱신 SQL 은 place-upsert 1벌(upsertTranslationWith)을 쓴다. 실패는 여정 응답을 막지 않는다
async function upsertTranslation(
  ...
  try {
    const { upsertTranslationWith } = await import("../place-upsert");
    ...
    console.warn(
      `[AG3-SAVE] ⚠️ 번역행 저장 실패 #${placeId}/${language}:`,
      (e as Error).message,
    );
  }
}
...
export async function saveNewPlacesToDB(
  ...
  opts?: { deferPersist?: boolean; language?: string },
): Promise<void> {
  if (!db || !cityId) {
    console.log(
  ...
  // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = Gemini 문구(k·r·s) 저장 갈래 = ko(기본) → PSR name_ko·summary_ko·editorial_summary / 그 외 언어 → place_translations(place_id, language) = 한국어 컬럼 오염 0 (정본 §)
  const outLang = (LANGS as readonly string[]).includes(opts?.language || "")
    ? (opts!.language as string)
    : "ko";
  const koCols = outLang === "ko";
  ...
  const stage1 = await Promise.all(
    toSave.map(async (place: any) => {
      ...
      const job = {
        ...
        nameKo: koCols ? ((place as any).nameKo ?? null) : null,
        ...
        shortformKo: koCols ? (place.description ?? null) : null, // → editorial_summary
        selectionReasonKo: koCols
      ...
      try {
        ...
        if (door) {
          ...
          // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 손님상 먼저 = 문이 찾은 행의 사진·PID·지도주소·리뷰수·좌표를 창고 쓰기 전에 슬롯에 붙인다(쓰기가 실패해도 있는 것은 100% 나간다). 유료 호출은 신규뿐 (정본 §)
          const row = door.row as any;
          (place as any).psrRowId = row.id;
          if (!place.image && row.image_url) place.image = row.image_url;
          if (!(place as any).googlePlaceId && row.google_place_id)
            (place as any).googlePlaceId = row.google_place_id;
          if (!(place as any).googleMapsUri && row.google_maps_uri)
            (place as any).googleMapsUri = row.google_maps_uri;
          if (
            !(place as any).userRatingCount &&
            row.google_review_count != null
          )
            (place as any).userRatingCount = row.google_review_count;
          if ((!place.lat || place.lat === 0) && row.lat && row.lng) {
            place.lat = row.lat;
            place.lng = row.lng;
          }
        }
        ...
        console.error(
          `[AG3-SAVE] ❌ "${place.name}" Gemini upsert 실패${(place as any).psrRowId ? ` (문이 찾은 #${(place as any).psrRowId} 재료는 슬롯에 이미 붙음 = 손님상 정상, 창고 쓰기만 후처리 몫)` : ""}:`,
          (e as Error).message,
        );
        return {
          place,
  ...
  // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 흡수행 결손 판정 = **PID 도 CID 도 없음**. CID 가 있다 = 이미 구글맵 페이지로 확인한 행 = TS 를 다시 부를 이유 0(5차 실측 TS 2·PM 2 낭비 차단). 옛 "PID 없음만" 폐기 §19.
  ...
  if (updatedRows.length > 0 && db) {
    ...
    const missById = new Map(
      chk.map((r: any) => [
        r.id,
        !r.googlePlaceId && !/cid=\d+/.test(String(r.googleMapsUri || "")),
      ]),
    ); // 결손 = PID·CID 둘 다 없음(이미지 무관)
    absorbedRows = updatedRows.filter((r: any) => missById.get(r.rowId));
  }
  // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = mode = raw 산출물(tsResults, §18) 라벨 + 이번 판에 만든 행인가(rowIsNew = TS 못 채운 신규 행만 dropRow). 쌍둥이 흡수에서 진 행은 신규·기존 가리지 않고 삭제(place-upsert 1벌) (정본 §)
  const tsTargets = [
    ...newRows.map((r: any) => ({ ...r, mode: "new" as const })),
    ...absorbedRows.map((r: any) => ({ ...r, mode: "absorbed" as const })),
  ];
  console.log(
    `[AG3-SAVE] ② TS 대상 = 신규 ${newRows.length} + 흡수(PID·CID 둘 다 없음) ${absorbedRows.length} = ${tsTargets.length}곳 (PID 또는 CID 있는 매칭행 ${updatedRows.length - absorbedRows.length}곳 = 유료호출 0, 사진·검증은 후처리 큐)`,
  );
    ...
    tsTargets.map(async ({ place, seedCategory, rowId, mode }: any) => {
      try {
        ...
        const tsArr = await tsSearch({
          ...
          // ⚠️ 2026-07-06 §18 = 건건 raw 로컬 skip(Storage 건건은 관문이 보존) = 아래 tsResults 모음 1파일이 로컬 조회용.
          localSkipRaw: true,
        });
        ...
        const jobBase = {
          ...
          shortformKo: koCols ? (place.description ?? null) : null,
          selectionReasonKo: koCols
            ? (place.personaFitReason ?? place.description ?? null)
            : null,
        ...
        const doUpdate = async () => {
          try {
            ...
            if (!koCols && u.rowId != null && u.rowId !== rowId)
              await upsertTranslation(
                u.rowId,
                outLang,
                place.personaFitReason ?? place.description ?? null,
