...
export async function finalizeDbOnlyItinerary(input: AG4DbInput): Promise<any> {
  ...
  if (cityId && db && !input.restaurantPool) {
    ...
    const pinCond = pinIdsForMeals.length
      ...
      : sql.raw("FALSE");
    // ⚠️ 수정금지(승인필요) 2026-08-31 사장님 확정 = 식당 풀 = 가격대 쿼터 없이 손님상 가능한 전체 (정본 B4)
    //   = 손님상 기준 = 구글이미지 + PID(돈이 들어간 결손 없음). 옛 10/30/20=60 쿼터는 시드 발굴 기준이라 폐기 §19.
    //   = 몇 곳을 캘까(발굴)와 있는 걸 어떻게 보여줄까(서빙)는 다른 문제. 좁히기는 여정엔진이 거리·베스트·RC·예산으로 한다.
    ...
    console.log(
      `[AG4-DB] 🍽️ 식당풀 DB 조회 = ${restaurantPool.length}곳 (= 손님상 가능 전체 = 구글이미지+PID+가격, 쿼터 없음)`,
    );
  }
  ...
  // ⚠️ 수정금지(승인필요) 2026-08-31 사장님 확정 = 식당풀도 조회 대상 = 식당 베스트 표식이 응답에서 빠지지 않게 (정본 B4)
  ...
  for (let d = 1; d <= dayCount; d++) {
    ...
    const dayPlaces = scenes.map((scene, sceneIdx) => {
      ...
      return {
        ...
        // ⚠️ 수정금지(승인필요) 2026-08-31 사장님 결정 = 베스트 언어코드를 화면·검수에 그대로 내려보냄 (정본 B4)
        bestRank: inputPlace?.bestRank ?? null,
