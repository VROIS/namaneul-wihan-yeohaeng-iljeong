...
import { type Result, type Row } from "./gmaps-pid-identity/gates";
import {
  PID_ROWS_SELECT,
  PID_ROWS_WHERE,
  verifyPidRows,
...
(async () => {
  ...
  const { PHOTO_MAX_WIDTH_PX } = await import(
    pathToFileURL(path.join(ROOT, "worker/lib/services/shared/ts-client.ts"))
      .href
  );
  ...
  // ⚠️ 수정금지(승인필요) 2026-09-13 사장님 결정 = 대상 = PID 있는 전 행(bts_* 제외) · 사진은 전부 구글(R2) = 위키·외부 주소는 "사진 없음" 으로 보고 같은 방문에서 구글 사진으로 갈아끼운다. 조회 SQL·재확인 루프 = gmaps-pid-identity/run 1벌(후처리 큐 소비자와 공용) (정본 §)
  const r2Prefix = (process.env.R2_PUBLIC_URL || "").replace(/\/+$/, "") + "/";
  const params: any[] = [cityId, r2Prefix];
  let where = PID_ROWS_WHERE;
  if (ids.length) {
    params.push(ids);
    where += ` AND id = ANY($${params.length}::int[])`;
  }
  let rows: Row[] = (
    await c.query(`${PID_ROWS_SELECT} WHERE ${where} ORDER BY id`, params)
  ).rows;
  ...
  try {
    results = await verifyPidRows({
      browser,
      rows,
      city: { name_en: city.name_en, lat: cityLat, lng: cityLng },
      lang,
      ...
      cityId,
      apply,
      onResult: (r) => console.log(JSON.stringify(r)),
    });
  } finally {
