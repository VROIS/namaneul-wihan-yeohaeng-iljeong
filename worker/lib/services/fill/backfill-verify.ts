...
export async function backfillImages(opts: {
  ...
  // ⚠️ 수정금지(승인필요) 2026-08-17 사장님 승인 = 키 발급 = 다른 전 컴포넌트와 동일하게
  ...
  if (freeGmaps.length) {
    ...
    const { evaluateRow, initResult, isWritable, nameTokens } = await import(
      pathToFileURL(
        path.join(ROOT, "worker/lib/services/fill/gmaps-pid-identity/gates.ts"),
      ).href
    );
    const { writeRow, pickPageName } = await import(
      pathToFileURL(
        path.join(ROOT, "worker/lib/services/fill/gmaps-pid-identity/apply.ts"),
      ).href
    );
    const { distanceKmFromCoords } = await import(
      pathToFileURL(
        path.join(ROOT, "worker/lib/services/shared/pool-radius.ts"),
      ).href
    );
  ...
  if (apply && rows.length) {
    ...
    if (far.length) {
      ...
      const r = spawnSync(
        ...
        [
          "tsx",
          path.join(ROOT, "worker/lib/services/fill/wrongcity-quarantine.ts"),
          `--ids=${far.join(",")}`,
          "--apply",
        ],
...
export async function mirrorWikiVenueImages(opts: {
  ...
  const { withQuotaRetry } = await import(
    pathToFileURL(path.join(ROOT, "worker/lib/services/shared/retry-429.ts"))
      .href
  );
