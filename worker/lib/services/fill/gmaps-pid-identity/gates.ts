...
export async function evaluateRow(
  ...
  // ⚠️ 수정금지(승인필요) 2026-08-28 사장님 지시 = 관문 2종(이름·좌표) = PID 오염(오매칭) 감지, 이름 관문은 양쪽 토큰이 다 있을 때만(name_en 빈 행은 좌표 관문만) · name_en 이 한글(레거시 음역 데이터, 예 "안젤리나"="Angelina Paris - Rivoli")이면 프랑스어/영어 페이지 h1 과 토큰이 문자체계상 절대 안 겹쳐 매번 오탐 name-mismatch 가 남 = 이런 행도 name_en 빈 행과 동일하게 대조 불가 처리(좌표 관문만).
