...
import {
  calculateUberBlackHourly, guideCostForDay, round2,
  type TransportPricingResult, type GuidePriceResult, type TransitPriceResult,
} from '../transport-pricing-service';
import { normalizeTravelStyle, type GeminiPlace, type GeminiDay } from './pipeline-v3-types';
...
export async function buildDayResult(d: number, deps: DayBuilderDeps): Promise<{ dayResult: any; dailyPerPersonEur: number }> {
  const {
