...
import { generateRecommendations, type isCityReady } from './ag2-gemini-recommender';
...
export async function runPipelineDbOnly(
  ...
  _mark('AG2');
  console.log(`[DB-Only] AG2-DB 완료 (${_timings['AG2'] - _timings['AG1']}ms): ${placesArr.length}곳 (= Gemini 0)`);
  ...
  console.log(`[DB-Only]   AG1: ${_timings['AG1']}ms / AG2-DB: ${_timings['AG2'] - _timings['AG1']}ms`);
  console.log(`[DB-Only]   AG3-DB: ${_timings['AG3-DB'] - _timings['AG2']}ms / 슬롯: ${_timings['SlotDistribute'] - _timings['AG3-DB']}ms / AG4-DB: ${_timings['AG4-DB'] - _timings['SlotDistribute']}ms`);
