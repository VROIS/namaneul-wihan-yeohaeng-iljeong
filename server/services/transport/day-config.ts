...
import type { TransportType, CompanionType, MobilityStyle, TravelStyle } from './constants';
...
export async function guideCostForDay(args: {
  ...
  const [startH, startM] = (args.dayConfig.startTime || '09:00').split(':').map(Number);
  const [endH, endM] = (args.dayConfig.endTime || '21:00').split(':').map(Number);
