...
const hav = (a1: number, o1: number, a2: number, o2: number): number => {
  ...
  const h =
    Math.sin(dl / 2) ** 2 +
    Math.cos(r(a1)) * Math.cos(r(a2)) * Math.sin(dn / 2) ** 2;
...
function std(nn: string, mid: string, date: string): string {
  let step = STEP[nn] || nn,
    rest = mid;
  if (nn === "05") {
    if (rest.startsWith("restaurant-reverify")) {
      step = "restaurant-reverify";
      rest = rest.slice(step.length);
    } else if (rest.startsWith("text-recategorize")) {
      step = "text-recategorize";
      rest = rest.slice(step.length);
    }
  } else if (STEP[nn] && rest.startsWith(STEP[nn]))
    rest = rest.slice(STEP[nn].length);
...
const isArchive = (base: string) =>
  /^restfill-sim-|^dup-deep-scan-|-test-|-100km-|^04-sim[AB]-|\.pretty\.json$|^05-text-recategorize-input-/.test(
    base,
  );
...
(async () => {
  ...
  const cities = (
    await c.query(
      `SELECT id, name_en, latitude::float8 AS lat, longitude::float8 AS lng FROM cities WHERE latitude IS NOT NULL`,
    )
  ).rows;
  const objs = (
    await c.query(
  ...
  const get = (name: string) =>
    fetch(`${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(name)}`, {
  ...
  for (const name of objs) {
    ...
    const top = parts[0];
    const seg2 = parts[1];
    const base = parts.slice(2).join("/") || parts.slice(1).join("/");
    ...
    if (/^\d+$/.test(seg2)) {
      ...
      if (top === "docs-raw") {
        ...
        plan.push({
          from: name,
          to: s ? `${seg2}/${s}` : `${seg2}/_archive/${baseName}`,
          act: s ? "rename" : "archive",
          city: seg2,
        });
      } else {
      ...
      try {
        ...
        else if (j.request?.latitude && j.request?.longitude)
          coord = { lat: j.request.latitude, lng: j.request.longitude };
      } catch {
        /* */
      }
      if (coord) {
        ...
        for (const ct of cities) {
          const km = hav(coord.lat, coord.lng, ct.lat, ct.lng);
          if (km < bk) {
            bk = km;
            best = ct;
          }
  ...
  for (const p of plan) {
    if (p.act === "rt-keep" || p.act === "skip-unknown") continue;
    (toGroups.get(p.to) || toGroups.set(p.to, []).get(p.to))!.push(p);
  }
  for (const [to, group] of toGroups) {
    ...
    for (const p of group) {
      try {
        const j = await (await get(p.from)).json();
        const crypto = await import("crypto");
        p._hash = crypto
          .createHash("md5")
          .update(JSON.stringify(j.raw ?? {}))
          .digest("hex");
      } catch {
        p._hash = p.from;
      } // 못 열면 from 으로 고유 취급(보존)
      withHash.push(p);
    }
  ...
  const tos = plan
    .filter(
      (p) =>
        p.act !== "rt-keep" && p.act !== "skip-unknown" && p.act !== "dup-skip",
    )
    .map((p) => p.to);
  const dupes = [...new Set(tos.filter((t, i) => tos.indexOf(t) !== i))];
  const cnt = (a: string) => plan.filter((p) => p.act === a).length;
  console.log(
    `═══ storage-raw-restructure ${apply ? "(APPLY)" : "(DRY)"} = 총 ${plan.length} ═══`,
  );
  console.log(
    `rename ${cnt("rename")} / archive ${cnt("archive")} / rt복구이동 ${cnt("rt-recover")} / rt보존 ${cnt("rt-keep")} / 중복삭제 ${cnt("dup-skip")} / unknown ${cnt("skip-unknown")}`,
  );
  console.log(`충돌(순번부여 후 잔여 dupes) = ${dupes.length}`);
  // 순번 부여된(버전) 항목 표시
  const versioned = plan.filter(
    (p) => /_\d+\.json$/.test(p.to || "") && p.act !== "dup-skip",
  );
  if (versioned.length) {
    console.log(
      `\n[버전 순번 부여] ${versioned.length}건 (= tag 구별 안 되는 다른 호출):`,
    );
    versioned.forEach((p) =>
      console.log(`  ${p.from.split("/").pop()}\n   → ${p.to}`),
    );
  }
  const dupSkipped = plan.filter((p) => p.act === "dup-skip");
  if (dupSkipped.length) {
    console.log(`\n[내용동일 중복삭제] ${dupSkipped.length}건:`);
    dupSkipped.forEach((p) =>
      console.log(`  ${p.from.split("/").pop()} ${p.to}`),
    );
  }
  ...
  let moved = 0,
    err = 0;
  for (const p of plan) {
    if (p.act === "rt-keep" || p.act === "skip-unknown" || p.from === p.to)
      continue;
    try {
      const body = Buffer.from(await (await get(p.from)).arrayBuffer());
      const put = await fetch(
        `${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(p.to)}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!put.ok) {
        err++;
        console.error(`  ✗ PUT ${p.to}: ${put.status}`);
        continue;
      }
      const del = await fetch(
        `${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(p.from)}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${KEY}` },
          signal: AbortSignal.timeout(20000),
      ...
      if (!del.ok) {
        console.error(`  ⚠️ PUT ok but DEL 실패 ${p.from}: ${del.status}`);
      }
      moved++;
    } catch (e: any) {
      err++;
