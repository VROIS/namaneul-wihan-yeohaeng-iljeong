...
(async () => {
  ...
  console.log(`═══ category-move (city ${cityId} ${city.name_en}) = 이동 후보 ${moves.length}건 ${apply ? '[APPLY]' : '[DRY]'} ═══`);
  if (!moves.length) { console.log('  이동 목록 없음 (= MOVES 에 도시 미등록)'); await c.end(); return; }

  if (apply) await c.query('BEGIN');
  try {
    for (const mv of moves) {
      if (EXCLUDE_BTS.includes(mv.to)) { console.warn(`  ✗ skip: ${mv.name} → ${mv.to} (BTS 카테고리 이동 금지)`); continue; }
      ...
      if (!rows.length) { console.warn(`  ✗ 미발견: "${mv.name}" (= 이미 이동됐거나 이름 불일치)`); continue; }
      for (const r of rows) {
        if (r.seed_category === mv.to) { console.log(`  = 이미 ${mv.to}: ${mv.name} (id=${r.id}) skip`); continue; }
        const maxRank = (await c.query(`SELECT COALESCE(MAX(rank), 0)::int AS m FROM place_seed_raw WHERE city_id=$1 AND seed_category=$2`, [cityId, mv.to])).rows[0].m;
        const newRank = maxRank + 1;
        console.log(`  ${apply ? '✓' : '·'} ${mv.name} (id=${r.id}) : ${r.seed_category}#${r.rank} → ${mv.to}#${newRank} (RC=${r.google_review_count ?? 'NULL'})`);
        if (apply) {
          await c.query(
    ...
    if (apply) { await c.query('COMMIT'); console.log('✓ 이동 완료 (DELETE 0 = 데이터 보존)'); }
