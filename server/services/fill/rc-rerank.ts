...
(async () => {
  ...
  const prev = (await c.query(`
    ...
    SELECT COUNT(*)::int AS total, COUNT(*) FILTER (WHERE old_rank IS DISTINCT FROM new_rank)::int AS changed FROM ranked`, [cityId])).rows[0];
  console.log(`═══ rc-rerank (city ${cityId} ${city.name_en}) = 대상 ${prev.total} / 변경예정 ${prev.changed} ═══`);
  ...
  if (!apply) {
    const sample = (await c.query(`
      ...
      WHERE seed_category IN ('hotspot','heritage') AND new_rank <= 8 ORDER BY seed_category, new_rank`, [cityId])).rows;
    console.log('[DRY 미리보기 = hotspot/heritage TOP8 (새rank ← 옛rank | RC)]');
    for (const r of sample) console.log(`  ${r.seed_category} #${r.new_rank} ← ${r.old_rank} | ${r.name_en} | RC ${r.rc ?? 'NULL'}`);
    console.log('\n=== DRY (--apply 로 실행) ===');
    await c.end(); return;
  }
  ...
  await c.query('BEGIN');
  try {
    await c.query(`UPDATE place_seed_raw SET rank = -id WHERE city_id=$1 AND ${EXCLUDE}`, [cityId]);
    await c.query(`
      WITH ranked AS (
        SELECT id, ROW_NUMBER() OVER (PARTITION BY seed_category ORDER BY ${ORDER}) AS rk
        FROM place_seed_raw WHERE city_id=$1 AND ${EXCLUDE}
      )
      UPDATE place_seed_raw SET rank = ranked.rk FROM ranked WHERE place_seed_raw.id = ranked.id`, [cityId]);
