...
(async () => {
  ...
  if (apply && !storageKey) {
    console.error("✗ SUPABASE_SERVICE_ROLE_KEY 미존재 = 삭제 불가");
    process.exit(1);
  }
  ...
  console.log(
    `═══ storage-orphan-cleanup (city ${cityId}) = 고아 ${orphans.length} (부모有중복 ${dup} / 부모無죽은 ${dead}) ═══`,
  );
  if (!apply) {
    console.log(
      `[샘플] ${orphans
  ...
  for (let i = 0; i < names.length; i += 100) {
    ...
    if (r.ok) del += batch.length;
    else {
      err += batch.length;
      console.log(
        `  ✗ batch ${i}: ${r.status} ${(await r.text()).slice(0, 100)}`,
      );
    }
  }
  console.log(`\n═══ 결과 = 삭제 ${del} / 실패 ${err} ═══`);
