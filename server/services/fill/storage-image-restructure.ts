...
(async () => {
  ...
  const rows = (await c.query(`
    ...
    ORDER BY oi.city_id, oi.row_id`, [BUCKET])).rows;
  ...
  const stdUrl = (r: any) => `${SUPA}/storage/v1/object/public/${BUCKET}/${stdObj(r)}`;
  ...
  console.log(`═══ storage-image-restructure ${apply ? '(APPLY)' : '(DRY)'} = 옛형식 ${rows.length} (${onlyCity ? 'city ' + onlyCity : '전 도시'}) ═══`);
  console.log(`표준화 가능(PID有) ${movable.length} / 건너뜀(PID無) ${skipNoPid.length}`);
  const byCity: Record<string, number> = {};
  for (const r of movable) byCity[r.city_id] = (byCity[r.city_id] || 0) + 1;
  console.log(`도시별 = ${Object.entries(byCity).map(([k, v]) => `${k}:${v}`).join(' / ')}`);
  console.log('\n[표준화 매핑 샘플]');
  movable.slice(0, 5).forEach((r: any) => console.log(`  ${r.old_obj} → ${stdObj(r)} (${r.name_en})`));
  if (skipNoPid.length) { console.log('\n[PID無 건너뜀]'); skipNoPid.forEach((r: any) => console.log(`  ${r.old_obj} (row ${r.row_id} ${r.name_en || 'row없음'})`)); }

  if (!apply) { await c.end(); console.log('\n=== DRY 완료 (--apply 로 이동+DB갱신) ==='); return; }
  if (!KEY) { await c.end(); console.error('✗ Storage KEY 미비'); return; }

  const { upsertPlace } = await import(pathToFileURL(path.join(ROOT, 'server/services/place-upsert.ts')).href);
  let moved = 0, dbUpd = 0, err = 0;
  for (const r of movable) {
    try {
      // 표준경로에 이미 같은 파일 있으면 다운로드 생략(중복 방지) — 없으면 옛것 복사
      const dst = stdObj(r);
      const head = await fetch(`${SUPA}/storage/v1/object/info/${BUCKET}/${encodeURI(dst)}`, { headers: { Authorization: `Bearer ${KEY}` }, signal: AbortSignal.timeout(15000) });
      if (!head.ok) {
        const body = Buffer.from(await (await fetch(`${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(r.old_obj)}`, { headers: { Authorization: `Bearer ${KEY}` }, signal: AbortSignal.timeout(20000) })).arrayBuffer());
        const put = await fetch(`${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(dst)}`, { method: 'PUT', headers: { Authorization: `Bearer ${KEY}`, 'Content-Type': 'image/jpeg', 'x-upsert': 'true' }, body, signal: AbortSignal.timeout(20000) });
        if (!put.ok) { err++; console.error(`  ✗ PUT ${dst}: ${put.status}`); continue; }
      }
      // DB image_url = 표준 URL 로 갱신 (upsertPlace = PID 매칭 = 기존행 UPDATE)
      const res = await upsertPlace({ cityId: r.city_id, seedCategory: r.cat, nameEn: r.name_en, googlePlaceId: r.pid, imageUrl: stdUrl(r) });
      if (res.action === 'updated' || res.action === 'inserted') dbUpd++;
      // 옛 파일 삭제
      const del = await fetch(`${SUPA}/storage/v1/object/${BUCKET}/${encodeURI(r.old_obj)}`, { method: 'DELETE', headers: { Authorization: `Bearer ${KEY}` }, signal: AbortSignal.timeout(15000) });
      if (del.ok) moved++;
      else console.error(`  ⚠️ 표준PUT+DB ok but 옛삭제 실패 ${r.old_obj}: ${del.status}`);
    } catch (e: any) { err++; console.error(`  ✗ ${r.old_obj}: ${e.message}`); }
  }
  await c.end();
