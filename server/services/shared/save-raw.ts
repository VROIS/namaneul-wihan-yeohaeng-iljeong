  ...
  try {
    ...
    try {
      ...
      if (fs.existsSync(localDir)) {
        ...
        const versioned: string = versionedName(localDir, stemFileName, newHash, (p: string) => { // 같은 stem 계열 중 내용동일=덮어쓰기 / 다름=_maxN+1
          try { return rawHash(JSON.parse(fs.readFileSync(p, 'utf8')).raw); } catch { return null; } // 기존 파일의 raw 부분만 rawHash (= 못 열면 null)
        });
