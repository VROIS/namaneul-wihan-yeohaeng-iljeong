...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/budget/preview", async (req, res) => {
    try {
      const { getQuickBudgetPreview } = await import(
        "./services/budget-calculator"
      );
  ...
  app.post("/api/budget/calculate", async (req, res) => {
    try {
      const { calculateTravelBudget } = await import(
        "./services/budget-calculator"
      );
      const {
        travelStyle,
  ...
  app.get("/api/itineraries/:id/video", async (req, res) => {
    try {
      ...
      // 완료, 부분 완료, 실패 상태
      if (
        itinerary.videoStatus === "succeeded" ||
