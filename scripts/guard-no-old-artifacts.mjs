...
const DELETE_MARK = /(폐기|폐지|삭제|철회|제거|완전삭제|deprecated|dead[- ]?code)/;
const DATE_OR_SEC = /(20\d\d-\d\d-\d\d|§\s?\d)/;
// CLAUDE.md = 헌법 자체 = 금지 패턴을 글로 설명해야 함 = 스캔 제외(가드가 자기 규칙 설명문을 잡으면 안 됨).
const SKIP_PATH   = /(server[\/\\]data[\/\\]|\.json$|node_modules|server_dist|dist[\/\\]|bts-app[\/\\]node_modules|_archive|CLAUDE\.md$|guard-no-old-artifacts\.mjs$)/;
...
const RULES = [
  { id: 'OLD_QUOTE_DQ',       re: /옛\s*"[^"]+"/,                          quote: true,  desc: '옛 "..." 옛 값 인용 박제' },
  { id: 'OLD_QUOTE_SQ',       re: /옛\s*'[^']+'/,                          quote: true,  desc: "옛 '...' 옛 값 인용 박제" },
  { id: 'OLD_QUOTE_BT',       re: /옛\s*`[^`]+`/,                          quote: true,  desc: '옛 `...` 옛 코드값 박제' },
  { id: 'OLD_ARRAY',          re: /옛\s*\[[^\]]+\]/,                       quote: true,  desc: '옛 [배열값] 박제' },
  { id: 'OLD_VS_NEW_COEXIST', re: /(이전|옛)\s*방식\s*=.*새\s*방식\s*=/,    quote: true,  desc: '옛/새 방식 모순 공존' },
  { id: 'OLD_FALLBACK',       re: /옛방식\s*\|\||\|\|\s*새방식|if\s*\(\s*옛/, quote: false, desc: '옛방식 || 새방식 폴백 분기 잔존' },
  { id: 'STRIKETHROUGH',      re: /~~[^~]+~~/,                            warn: true,   desc: '~~취소선~~ 박제 의심(경고)' },
];
...
function scanFile(path) {
  ...
  readFileSync(path, 'utf8').split(/\r?\n/).forEach((line, i) => {
    for (const h of scanLine(line))
      out.push({ path, line: i + 1, id: h.id, warn: !!h.warn, desc: h.desc, text: line.trim().slice(0, 100) });
  });
  return out;
}
...
function scanStaged() {
  const files = execSync('git diff --cached --name-only --diff-filter=ACM', { encoding: 'utf8' })
    .split('\n').filter(Boolean).filter((f) => SCAN_EXT.test(f) && !SKIP_PATH.test(f));
  const out = [];
  for (const f of files) {
    ...
    diff.split(/\r?\n/).forEach((l) => {
      ...
      for (const h of scanLine(line))
        out.push({ path: f, line: '+', id: h.id, warn: !!h.warn, desc: h.desc, text: line.trim().slice(0, 100) });
    });
  }
  return out;
}
...
function fileFromStdin() {
  try {
    ...
    return j?.tool_input?.file_path || j?.file_path || j?.tool_input?.path || null;
  } catch { return null; }
}

// --all: 레포 전체 추적 파일 스캔 (= 현황 점검용 = 한 프로세스에서 git ls-files 일괄)
function scanAll() {
  const files = execSync('git ls-files "*.ts" "*.tsx" "*.js" "*.mjs" "*.cjs" "*.md" "*.sql"', { encoding: 'utf8' })
...
else { console.error('사용법: --staged | --all | --file <경로> | --stdin'); process.exit(2); }
...
if (warn.length) {
  ...
  for (const w of warn) console.error(`  ${w.path}:${w.line} [${w.id}] ${w.text}`);
}
if (hard.length) {
  console.error('\n❌ §19 위반 = 옛것 박제 차단 (헌법: 옛것은 완전삭제, 삭제사유는 "폐기 + 날짜/§" 1줄로):');
  for (const h of hard) console.error(`  ${h.path}:${h.line} [${h.id}] ${h.desc}\n     → ${h.text}`);
