...
if (!SUPA_URL || !SR_KEY) { console.error('❌ SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY 필요'); process.exit(1); }
...
console.log(`고아 대상 = ${paths.length}개 / ${(totalBytes / 1024 / 1024).toFixed(1)}MB`);
...
const supa = createClient(SUPA_URL, SR_KEY, { auth: { persistSession: false } });
let deleted = 0, failed = 0;
for (let i = 0; i < paths.length; i += 100) {
  const batch = paths.slice(i, i + 100);
  const { data, error } = await supa.storage.from(BUCKET).remove(batch);
  if (error) { console.error(`  ❌ 배치 ${i / 100 + 1} 실패: ${error.message}`); failed += batch.length; }
  else { deleted += (data?.length ?? batch.length); console.log(`  ✅ 배치 ${i / 100 + 1}: ${data?.length ?? batch.length}개 삭제 (누적 ${deleted})`); }
}
