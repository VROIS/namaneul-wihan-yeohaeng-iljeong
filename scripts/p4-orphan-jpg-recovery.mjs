...
try {
  ...
  console.log(`📦 Storage 총: ${sf.rows.length} 파일 / Orphan (DB row 없음): ${orphans.length}\n`);
  if (orphans.length === 0) {
  ...
  for (const runId of RUN_IDS) {
    let logText = '';
    try {
      logText = execSync(`gh run view ${runId} --log`, { encoding: 'utf8', maxBuffer: 50 * 1024 * 1024 });
    } catch (e) {
      console.error(`   ❌ run ${runId} 로그 추출 실패: ${e.message.slice(0, 100)}`);
      continue;
    }
  ...
  let supabaseAnonKey = null, supabasePublicUrl = null;
  if (COMMIT) {
    const k1 = await db.query("SELECT key_value FROM api_keys WHERE key_name='SUPABASE_ANON_KEY' AND is_active=true LIMIT 1");
    supabaseAnonKey = k1.rows[0]?.key_value;
    if (!supabaseAnonKey) throw new Error('SUPABASE_ANON_KEY 없음');
    // SUPABASE URL 추출
    const m = SUPA_URL.match(/db\.([a-z0-9]+)\.supabase\.co/) || SUPA_URL.match(/postgres\.([a-z0-9]+):/);
  ...
  for (const orphan of orphans) {
    ...
    if (!newRowId) {
      unmatched.push({ ...orphan, reason: `살아있는 row 없음 ("${oldInfo.name}")` });
      continue;
    }
    matched.push({
  ...
  if (matched.length > 0) {
    ...
    for (const m of matched) {
      ...
      try {
        // Storage move
        await storageMove(supabasePublicUrl, supabaseAnonKey, m.old_path, m.new_path);
  ...
  console.log(`모드                      : ${DRY_RUN ? 'DRY-RUN (DB/Storage 변경 X)' : 'COMMIT 완료'}`);
  console.log('═'.repeat(80));
