...
console.log(`Mode: ${DRY ? 'DRY RUN' : '🔴 APPLY'} ${FORCE ? '(FORCE = 백업 후 강제)' : ''}\n`);
...
if (FORCE && !DRY) {
  ...
  if (!fs.existsSync(backupPath)) {
    ...
    console.error(`   먼저 = node scripts/backup-place-seed-raw-43-cols.mjs 실행`);
    process.exit(1);
  }
  const stat = fs.statSync(backupPath);
  console.log(`✅ 백업 확인: ${backupPath} (${(stat.size / 1024).toFixed(1)} KB)\n`);
}
...
const colsBefore = await c.query(`SELECT COUNT(*) as n FROM information_schema.columns WHERE table_name='place_seed_raw'`);
...
for (const col of DEAD_COLS) {
  try {
    const r = await c.query(`SELECT COUNT(*) FILTER (WHERE ${col} IS NOT NULL) AS n FROM place_seed_raw`);
...
if (DRY) {
  ...
  console.log(`\n예정 SQL:`);
  console.log(`BEGIN;`);
  DEAD_COLS.forEach(c => console.log(`  ALTER TABLE place_seed_raw DROP COLUMN IF EXISTS ${c};`));
  console.log(`COMMIT;`);
  await c.end();
  process.exit(0);
}
...
try {
  ...
  const colsAfter = await c.query(`SELECT COUNT(*) as n FROM information_schema.columns WHERE table_name='place_seed_raw'`);
  console.log(`\n[검증]`);
  console.log(`  row 수: ${after.rows[0].n} (= 변화 없음 ✅)`);
  console.log(`  컬럼 수: ${colsBefore.rows[0].n} → ${colsAfter.rows[0].n} (= ${colsBefore.rows[0].n - colsAfter.rows[0].n} 삭제)`);
