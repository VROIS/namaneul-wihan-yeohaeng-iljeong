...
async function checkAllTables() {
  ...
  try {
    ...
    const cities = await client.query(
      "SELECT id, name, country FROM cities ORDER BY id LIMIT 50",
    );
    cities.rows.forEach((r) => {
      const broken = hasBrokenKorean(r.name) || hasBrokenKorean(r.country);
      console.log(
        (broken ? "X " : "  ") + "[" + r.id + "] " + r.name + ", " + r.country,
      );
    });
    ...
    const places = await client.query(
      "SELECT id, name FROM places ORDER BY id LIMIT 50",
    );
    ...
    const ytChannels = await client.query(
      "SELECT id, channel_id, channel_name FROM youtube_channels ORDER BY id LIMIT 50",
    );
    ...
    const igHashtags = await client.query(
      "SELECT id, hashtag FROM instagram_hashtags ORDER BY id LIMIT 50",
    );
    ...
    const blogs = await client.query(
      "SELECT id, source_name, platform, source_url FROM blog_sources ORDER BY id LIMIT 50",
    );
    ...
    const naverPosts = await client.query(
      "SELECT id, title, author FROM naver_blog_posts ORDER BY id LIMIT 20",
    );
    ...
    const naverCount = await client.query(
      "SELECT COUNT(*) as cnt FROM naver_blog_posts",
    );
    ...
    const ytVideos = await client.query(
      "SELECT id, title FROM youtube_videos ORDER BY id LIMIT 20",
    );
    ...
    const ytCount = await client.query(
      "SELECT COUNT(*) as cnt FROM youtube_videos",
    );
    ...
    const brokenCities = cities.rows.filter(
      (r) => hasBrokenKorean(r.name) || hasBrokenKorean(r.country),
    ).length;
    const brokenPlaces = places.rows.filter((r) =>
      hasBrokenKorean(r.name),
    ).length;
    const brokenYT = ytChannels.rows.filter((r) =>
      hasBrokenKorean(r.channel_name),
    ).length;
    const brokenIG = igHashtags.rows.filter((r) =>
      hasBrokenKorean(r.hashtag),
    ).length;
    const brokenBlogs = blogs.rows.filter((r) =>
      hasBrokenKorean(r.source_name),
    ).length;
    const brokenNaver = naverPosts.rows.filter((r) =>
      hasBrokenKorean(r.title),
    ).length;
    const brokenVideos = ytVideos.rows.filter((r) =>
      hasBrokenKorean(r.title),
    ).length;

    console.log(
      "cities: " + brokenCities + " broken / " + cities.rows.length + " total",
    );
    console.log(
      "places: " + brokenPlaces + " broken / " + places.rows.length + " total",
    );
    console.log(
      "youtube_channels: " +
        brokenYT +
        " broken / " +
        ytChannels.rows.length +
        " total",
    );
    console.log(
      "instagram_hashtags: " +
