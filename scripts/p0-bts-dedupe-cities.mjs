...
async function run() {
  ...
  try {
    ...
    console.log(`  cities 삭제: ${citiesDel.rowCount} rows (id=${removeIds.join(',')})`);
    ...
    if (!SKIP_RANK_REMAP) {
      ...
      for (const remap of RANK_REMAP) {
        ...
        console.log(`  ${remap.cityName}: ${remap.oldRank} → ${remap.newRank} (rows: ${r.rowCount})`);
      }
    } else {
...
async function processPair(db, { remove, keep, removeName, keepName }) {
  ...
  console.log(`  Step 1. 중복 장소 삭제 (master 에 이미 있음): ${dupDel.rowCount} rows`);
  ...
  console.log(`  Step 2. 고유 장소 이동 (rank 재할당): ${moveRes.rowCount} rows`);
  ...
  console.log(`  Step 3. master city 전체 rank 재정렬: ${remapRes.rowCount} rows`);
  ...
  dist.rows.forEach((r) => console.log(`    ${r.seed_category.padEnd(12)} ${r.n}`));
