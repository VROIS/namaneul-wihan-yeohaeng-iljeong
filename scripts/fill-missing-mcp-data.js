...
async function enrichMissingData() {
    ...
    `);
        ...
        console.log(`📊 총 ${res.rows.length}개의 시티/카테고리 조합에서 누락 발견.`);
        ...
        for (const row of res.rows) {
            console.log(`📌 [도시 ID: ${row.city_id}, 카테고리: ${row.seed_category}] - 누락 건수: ${row.missing_count}`);
        ...
        console.log('이미 개선된 Stage 1 로직이 배포되었으므로, 재수집 시 빈칸이 채워질 것입니다.');
