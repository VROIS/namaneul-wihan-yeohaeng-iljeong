...
const CAPABILITIES = [
  {
    id: 'image-fill', ko: '이미지 결손 채우기(PID 있고 사진만 없는 행 → PM 일괄)',
    ...
    owners: [
      'server/services/fill/image-backfill.ts', 'server/services/fill/storage-image-relink.ts',
      'server/services/shared/ts-client.ts', 'fillcity/repair.ts', 'server/services/agents/ag3-data-matcher.ts',
      'fillcity/prompts/12-ts-discover-pool/post-process.ts', 'fillcity/prompts/06-ts-pm-enrich/post-process.ts',
  ...
  {
    id: 'ts-backfill', ko: 'PID 없는 행 TS 재검증·9요소 보강',
    owners: ['server/services/fill/ts-backfill.ts'],
    triggers: [/PID\s*없는.*tsSearch/, /google_place_id\s+IS\s+NULL[\s\S]{0,80}tsSearch/i],
    hint: 'PID 없는 행 보강 = ts-backfill.ts. 새 TS 재검증 루프 만들지 말 것(§16).',
  },
  {
    id: 'rc-rerank', ko: '최종 랭킹(RC DESC) 재정렬',
    owners: ['server/services/fill/rc-rerank.ts'],
    triggers: [/rank\s*=[\s\S]{0,40}(ROW_NUMBER|RANK\s*\(\)|ORDER BY.*google_review_count)/i],
    hint: '랭킹은 DB autorank 트리거 + rc-rerank.ts 단일 권위체. 앱 코드에서 rank 계산 금지(§19 이중화).',
  },
  {
    id: 'raw-reinsert', ko: '저장 raw → PSR 재입력(외부호출 0)',
  ...
  {
    id: 'category-move', ko: '오분류 장소 카테고리 이동(DELETE 금지)',
...
function buildCatalog() {
  const lines = ['# server/services/fill/ 결손별 단독 도구 카탈로그 (= §16 재발명 금지 = 기존 것 재사용)', ''];
  if (existsSync(FILL_DIR)) {
    for (const f of readdirSync(FILL_DIR).filter((x) => x.endsWith('.ts')).sort()) {
      const head = readFileSync(join(FILL_DIR, f), 'utf8').split(/\r?\n/).find((l) => l.trim().startsWith('//')) || '';
      const resp = head.replace(/^\/\/\s*⚠️?\s*(수정금지\(승인필요\)|영구 컴포넌트)?\s*20\d\d-\d\d-\d\d\s*(사장님|사용자)?\s*SSOT?\s*=?\s*/, '').trim();
      lines.push(`- server/services/fill/${f} = ${resp || '(헤더 1줄 없음)'}`);
    }
  }
  lines.push('', '# 새 결손 도구가 필요하면: 위 목록에 겹치는 게 없을 때만 fill/ 에 새 파일 + 이 가드 CAPABILITIES 에 1줄 등재.');
  return lines.join('\n');
}
...
function scanAddedLines(file, addedLines) {
  ...
  for (const cap of CAPABILITIES) {
    ...
    if (cap.owners.some((o) => o.endsWith('/') ? norm.includes(o) : norm.endsWith(o))) continue;
    for (const line of addedLines) {
      if (cap.triggers.some((re) => re.test(line))) {
        out.push({ file, cap: cap.ko, hint: cap.hint, text: line.trim().slice(0, 90) });
        break; // 능력당 1건
      }
    }
  }
...
const SKIP = /(node_modules|server_dist|dist[\/\\]|_archive|\.json$|guard-no-reinvention\.mjs$)/;
const SCAN_EXT = /\.(ts|tsx|js|mjs|cjs)$/;

function scanStaged() {
  const files = execSync('git diff --cached --name-only --diff-filter=ACM', { encoding: 'utf8' })
    .split('\n').filter(Boolean).filter((f) => SCAN_EXT.test(f) && !SKIP.test(f));
  const out = [];
  for (const f of files) {
    if (!existsSync(f)) continue;
    const diff = execSync(`git diff --cached -U0 -- "${f}"`, { encoding: 'utf8' });
...
function scanFile(path) {
  if (!path || !existsSync(path) || statSync(path).isDirectory() || SKIP.test(path) || !SCAN_EXT.test(path)) return [];
  const lines = readFileSync(path, 'utf8').split(/\r?\n/);
  return scanAddedLines(path, lines);
}

function fileFromStdin() {
  try { const j = JSON.parse(readFileSync(0, 'utf8') || '{}'); return j?.tool_input?.file_path || j?.file_path || null; } catch { return null; }
}

const args = process.argv.slice(2);
if (args[0] === '--catalog') { console.log(buildCatalog()); process.exit(0); }
...
else { console.error('사용법: --catalog | --staged | --file <경로> | --stdin'); process.exit(2); }

if (findings.length) {
  console.error('\n❌ §16 재발명 의심 = 이미 있는 도구를 다시 만들고 있습니다 (기존 것을 재사용하세요):');
  for (const h of findings) console.error(`  ${h.path || h.file} → [${h.cap}]\n     기존: ${h.hint}\n     감지: ${h.text}`);
  console.error('\n전체 도구 카탈로그: node scripts/guard-no-reinvention.mjs --catalog');
  console.error('정말 새 도구가 맞으면(기존과 능력이 다르면) 이 스크립트 CAPABILITIES 에 등재 후 재커밋.\n');
  process.exit(1);
}
process.exit(0);
