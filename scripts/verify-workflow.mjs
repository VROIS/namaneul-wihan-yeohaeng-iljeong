...
const JUDGE_RULES =
  ...
  // ⚠️ 수정금지(승인필요) 2026-08-14 사장님 지시 = **이미 판정 끝난 것을 매 라운드 다시 올리지 마라.**
  //   실제 사고(2026-08-13~14): 같은 2건이 4라운드 연속 blockers 로 올라와 사장님이 매번 같은 판정을 반복하셨다.
  "\n\n⚠️ **기판정 규칙(필수)** — 사장님이 이미 판정하신 것은 **다시 blockers 에 넣지 마라**(summary 에도 쓰지 마라)." +
  "\n- 판정 위치 = diff 에 포함된 **정본 문서(docs/*.md)**. 거기에 **'사장님 판정'·'판단 대기'·'범위 밖'·'의도된'** 으로" +
  " 적힌 항목은 이미 끝난 얘기다. 코드만 보고 다시 올리지 마라 — **문서를 먼저 읽어라.**" +
  "\n- 사장님 확정(2026-08-13) = `DaySection.tsx` 숙소 배지 문구([숙소 설정]/[숙소 변경]) = **의도된 로직 = 수정 불필요.**" +
  "\n- 사장님 확정(2026-08-13) = `DailyTotal.tsx` [바로가기] 딥링크 출발/도착 표기 = **§3 보호파일 + 판단 대기** = 지적·수정 대상 아님." +
  "\n- 사장님 확정(2026-08-13) = MIX(`pipeline-v3-day-builder.ts`)의 숙소 한국어 이름 = **이전부터 있던 별건 = 범위 밖.**" +
  "\n- 정말 새로 발견한 것만 올려라. 지난 라운드와 같은 지적이면 그건 **네가 문서를 안 읽은 것**이다." +
  // ⚠️ 수정금지(승인필요) 2026-09-14 사장님 결정 = 이번 턴의 사장님 판정·외부 근거를 검수관에게 그대로 준다 (정본 §22)
  (typeof A.context === "string" && A.context.trim()
    ? "\n\n⚠️ **이번 턴 사장님 판정·확정 사실(아래는 이미 결정된 것 = 다시 지적하지 마라):**\n" +
      A.context.trim()
    : "");
