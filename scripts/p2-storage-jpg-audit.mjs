...
try {
  ...
  if (sf.rows.length === 0) {
    console.log(
      '⚠️  파일 0건 — 선임 보고 "39 JPG 저장됨" = 거짓이거나 다른 버킷',
    );
  }
  ...
  for (const r of sf.rows) {
    ...
    byCategory[cat].push({
      row_id: m[2],
      size_kb: Math.round(parseInt(r.size || "0") / 1024),
    });
  }
  console.log("\n📊 카테고리별 JPG 파일 수:");
  for (const [cat, files] of Object.entries(byCategory).sort()) {
    console.log(
      `   ${cat.padEnd(14)} ${String(files.length).padStart(3)} 개 (총 ${files.reduce((a, b) => a + b.size_kb, 0)} KB)`,
    );
  }
  ...
  if (orphans.length === 0) {
    ...
    for (const o of orphans)
      console.log(`   - ${o.file} | ${o.status} | ${o.name_en}`);
  }
  ...
  if (broken.length === 0) {
    ...
    for (const b of broken)
      console.log(`   - row ${b.id} | ${b.name_en} | ${b.path}`);
  }
  ...
  console.log(
    `DB image_url storage URL 연결        : ${linkedRows.rows.length} 개`,
  );
