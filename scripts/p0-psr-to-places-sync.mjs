...
async function getTargetCities(db) {
  if (CITY_ID) {
    ...
    if (r.rows.length === 0) throw new Error(`city_id ${CITY_ID} 존재하지 않음`);
    return r.rows;
  }
  if (ALL_CITIES) {
    const r = await db.query(`
...
async function processCity(db, city) {
  ...
  for (const psr of psrRes.rows) {
    for (const pl of plRes.rows) {
      ...
      if (psr.latitude && psr.longitude && pl.latitude && pl.longitude) {
        const dKm = haversineKm(psr.latitude, psr.longitude, pl.latitude, pl.longitude);
        if (dKm > COORD_THRESHOLD_KM) {
          console.log(`  ⚠️  이름 일치하나 좌표 ${dKm.toFixed(2)}km > ${COORD_THRESHOLD_KM}km: ${psr.name_en}`);
          continue;
        }
      }
  ...
  console.log(`  📷 이미지 주입: ${imageInjected} (중복/URL없음 skip: ${imageSkipped})`);

  // 신규 INSERT (psr only)
  let inserted = 0, coordSkipped = 0;
  for (const psr of psrRes.rows) {
  ...
  console.log(`  ➕ 신규 INSERT: ${inserted} (좌표 없어 skip: ${coordSkipped})`);

  return { matched: matched.size, imageInjected, imageSkipped, inserted, coordSkipped };
}
...
async function run() {
  ...
  try {
    ...
    const totals = { matched: 0, imageInjected: 0, imageSkipped: 0, inserted: 0, coordSkipped: 0 };
    for (const city of cities) {
    ...
    console.log(`이미지 주입: ${totals.imageInjected} (skip: ${totals.imageSkipped})`);
    console.log(`신규 INSERT: ${totals.inserted} (좌표 없어 skip: ${totals.coordSkipped})`);
