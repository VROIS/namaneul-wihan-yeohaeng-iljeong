...
async function restoreData() {
  ...
  try {
    ...
    const placesSample = await client.query(
      "SELECT id, name, city_id FROM places ORDER BY id LIMIT 15",
    );
    placesSample.rows.forEach((r) =>
      console.log("  [" + r.id + "] " + r.name + " (city:" + r.city_id + ")"),
    );

    console.log("\ninstagram_hashtags sample:");
    const igSample = await client.query(
      "SELECT id, hashtag, category FROM instagram_hashtags ORDER BY id LIMIT 15",
    );
    igSample.rows.forEach((r) =>
      console.log("  [" + r.id + "] " + r.hashtag + " (" + r.category + ")"),
    );
  } catch (error) {
