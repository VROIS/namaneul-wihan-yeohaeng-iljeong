...
async function checkData() {
    ...
    if (stats[0].withBoth > 0) {
        const samples = await db.select().from(placeSeedRaw).where(sql`image_url is not null and price_eur > 0`).limit(5);
        console.log("📄 Samples (Both populated):");
        samples.forEach(s => {
            console.log(`- ${s.nameKo} (${s.nameEn}): Price=${s.priceEur}EUR, Image=${s.imageUrl?.substring(0, 50)}...`);
        });
    } else {
