...
async function main() {
  ...
  try {
    ...
    await client.query(
      `UPDATE guide_prices SET service_type = 'walking' WHERE id = 6`,
    );
    console.log("id=6 service_type 수정 완료");

    const result = await client.query(
      "SELECT id, service_type, service_name FROM guide_prices ORDER BY id",
    );
    console.log("\n현재 데이터:");
    result.rows.forEach((r) =>
      console.log(`  ${r.id}: ${r.service_type} - ${r.service_name}`),
    );
