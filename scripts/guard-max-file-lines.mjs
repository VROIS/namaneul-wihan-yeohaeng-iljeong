...
function catalog() {
  ...
  const tracked = execSync("git ls-files", {
    ...
    .split("\n")
    .filter(Boolean)
    .filter((f) => SCAN_EXT.test(f) && !SKIP.test(norm(f)));
  for (const rel of tracked) {
    const full = join(ROOT, rel);
    if (!existsSync(full)) continue;
    const lines = countLines(readFileSync(full, "utf8"));
    if (lines > MAX_LINES)
      out.push({ rel: norm(rel), lines, ex: isException(rel) });
  }
  ...
  for (const f of out)
    console.log(
      `${String(f.lines).padStart(6)}  ${f.rel}${f.ex ? "  [예외=데이터상수]" : ""}`,
    );
  return over.length;
}

// ── 순수 포맷변경 판정 = HEAD·staged 를 각각 prettier 포맷 후 같으면 코드(의미) 불변 = 포맷만. ──
//   = prettier 변형(따옴표·|위치·줄접기·후행쉼표)을 정규식 아닌 prettier 자체로 정확 판정. 실패 시 false(안전=차단).
//   ⚠️ prettier 를 npx 대신 API 직접 import = 매 파일 npx 재기동(수초×32회=타임아웃) 회피 = 프로세스 내 1회 로드 재사용.
...
async function isPrettierOnlyChange(f) {
  try {
    ...
    return false; // prettier 실패 = 안전하게 차단 유지
  }
}
...
async function scanStaged() {
  const files = execSync("git diff --cached --name-only --diff-filter=ACM", {
    ...
    .split("\n")
    .filter(Boolean)
    .filter((f) => SCAN_EXT.test(f) && !SKIP.test(norm(f)) && !isException(f));
  const violations = [];
  for (const f of files) {
    if (!existsSync(join(ROOT, f))) continue;
    // 커밋될 내용(= staged 버전) 기준으로 측정
    const stagedLines = countLines(
      execSync(`git show ":${f}"`, {
        encoding: "utf8",
        maxBuffer: 64 * 1024 * 1024,
      }),
    ...
    try {
      headLines = countLines(
        execSync(`git show "HEAD:${f}"`, {
          encoding: "utf8",
          maxBuffer: 64 * 1024 * 1024,
    ...
    if (headLines === null)
      violations.push({
        f,
        stagedLines,
        why: `신규 파일인데 ${stagedLines}줄 (한도 ${MAX_LINES})`,
      });
    else if (stagedLines > headLines) {
      // ⚠️ 2026-07-19 = 순수 포맷변경(prettier)이면 비대화 아님 = 통과.
      //   판정 = "HEAD 내용을 prettier 로 포맷한 결과 == staged 내용" 이면 = staged 는 HEAD 의 prettier 포맷본일 뿐 = 코드 불변 = 허용.
      //   (공백·따옴표·줄바꿈·|위치 등 prettier 변형을 정규식으로 흉내내지 않고 prettier 자체로 정확 판정.)
      if (await isPrettierOnlyChange(f)) continue;
      violations.push({
        f,
        stagedLines,
...
if (mode === "--staged") {
  const violations = await scanStaged();
  if (violations.length) {
    ...
    console.error(
      "데이터 전용 상수(로직 0)만 사장님 승인 후 이 스크립트 EXCEPTIONS 에 사유와 함께 등재.\n",
    );
    process.exit(1);
  }
  process.exit(0);
