...
try {
  ...
  if (orphanCheck.rows.length === 0) {
    ...
    console.log(
      `⚠️ ${orphanCheck.rows.length} / 39 orphan ID 가 DB 어딘가에 존재:`,
    );
  ...
  console.log("━".repeat(80));
  console.log(
    "파일명 = {row_id}.jpg → row_id 가 사라졌으므로 이름 매칭은 불가능.",
  );
  console.log(
  ...
  console.log(
    `복구 가능성       : ${orphanCheck.rows.length === 0 ? "0% (row_id 영구 소실)" : `${orphanCheck.rows.length}/39 가능`}`,
  );
  console.log("═".repeat(80));
} catch (e) {
