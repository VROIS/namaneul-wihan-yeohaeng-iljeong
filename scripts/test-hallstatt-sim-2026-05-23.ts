// ⚠️ 2026-05-23 = 사용자 명시 시뮬 = 신규 도시 = Hallstatt (= 오스트리아)
// 조건: 한국인 가족 / 모험 최우선 / 핫스팟 우선 / 빡빡 / 합리적 / 많이 걷기
// = 검증: cities INSERT + PSR INSERT + 응답 JSON 정상
...
async function main() {
  ...
  // BEFORE 스냅샷
  const before = await c.query(`SELECT COUNT(*)::int n FROM cities WHERE LOWER(name_en) = 'hallstatt'`);
  console.log(`\n[BEFORE] cities Hallstatt 행 = ${before.rows[0].n}`);

  const { runPipelineV3 } = await import('../server/services/agents/pipeline-v3.js');

  // 사용자 조건 (= 가족 / Adventure + Hotspot 두 vibe / Packed / Reasonable / Walking)
  const formData = {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`▶ Hallstatt 시뮬: 한국인 가족 4인 / 2일 / Adventure+Hotspot / 빡빡 / 합리적 / 도보`);
  ...
  try {
    ...
    fs.writeFileSync(`${outDir}/hallstatt-result.json`, JSON.stringify(result, null, 2));
  ...
  // 백그라운드 saveNewPlacesToDB 완료 대기 (= TS+PM+Storage+INSERT)
  console.log(`\n⏳ 백그라운드 saveNewPlacesToDB 완료 대기 (90초)...`);
  await new Promise(r => setTimeout(r, 90000));

  // AFTER 스냅샷
  const after = await c.query(`SELECT id, name, name_en, name_local, latitude, longitude FROM cities WHERE LOWER(name_en) = 'hallstatt'`);
  console.log(`\n[AFTER] cities Hallstatt 행 = ${after.rows.length}`);
  for (const r of after.rows) {
    console.log(`  cityId=${r.id} name="${r.name}" nameEn="${r.name_en}" 좌표=(${r.latitude}, ${r.longitude})`);
  }
  ...
  if (after.rows.length > 0) {
    ...
    for (const r of psrAfter.rows) {
      ...
      console.log(`  id=${r.id} cat=${r.seed_category.padEnd(11)} img=${r.has_img ? 'O' : 'X'} sum=${r.has_sum ? 'O' : 'X'} pid=${r.google_place_id ? 'O' : 'X'} price=€${r.price_eur || 0} tags=${JSON.stringify(r.phase_tags)} "${r.name_en}" [${cd}]`);
    }
  }
