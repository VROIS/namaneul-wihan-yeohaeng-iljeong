...
const normAddrSql = `LOWER(regexp_replace(regexp_replace(COALESCE($1, ''), '[.,;:!?''"()\\[\\]{}]', ' ', 'g'), '\\s+', ' ', 'g'))`;

const r = await c.query(`
  WITH n AS (
    SELECT id, city_id, google_place_id,
      latitude, longitude,
  ...
  dup_pid AS (
    ...
    WHERE a.google_place_id IS NOT NULL AND b.google_place_id IS NOT NULL
...
const tot = r.rows[0].pid_pairs + r.rows[0].addr_pairs + r.rows[0].coord_pairs + r.rows[0].name_pairs;
console.log(`\n총 중복 쌍 (4 단계 누적): ${tot}`);
await c.end();
