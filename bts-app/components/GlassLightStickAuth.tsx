...
export default function GlassLightStickAuth({
  onLoginComplete,
}: {
  ...
  return (
    <Animated.View style={[styles.container, animatedBackdrop]}>
      ...
      <Animated.View style={[styles.stickWrapper, animatedStick]}>
        {/* 아미봉 상단 헤드 (둥근 구형) */}
        <BlurView
          intensity={20 + progress.value * 80}
          tint={progress.value === 1.0 ? "light" : "dark"}
          style={styles.bombHead}
        >
          ...
          <View style={styles.inputSection}>
            <Text
              style={[styles.label, progress.value >= 0.5 && { color: "#FFF" }]}
            >
              생년월일 (1단계: 예열 스위치)
            </Text>
            <TextInput
        ...
        <BlurView
          intensity={20 + progress.value * 80}
          tint={progress.value >= 0.5 ? "default" : "dark"}
          style={styles.bombHandle}
        >
          <Text
            style={[
              styles.label,
              { marginBottom: 15 },
              progress.value >= 0.5 && { color: "#FFF" },
            ]}
          >
            2단계: 메인 스위치 점등
          </Text>
