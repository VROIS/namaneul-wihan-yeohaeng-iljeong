...
(async () => {
  ...
  const pidDup = (await c.query(`
    ...
    GROUP BY google_place_id HAVING COUNT(*) > 1 ORDER BY n DESC`, [cityId])).rows;
  console.log(`[1] PID 중복 = ${pidDup.length}그룹 (같은 구글ID 2+행 = 명백 중복):`);
  for (const r of pidDup.slice(0, 20)) console.log(`  PID(${r.n}) ids=${r.ids.join(',')} = ${r.names.join(' / ')}`);

  const nameDup = (await c.query(`
    ...
    GROUP BY 1 HAVING COUNT(*) > 1 ORDER BY n DESC`, [cityId])).rows;
  console.log(`\n[2] 이름 정규화 중복 = ${nameDup.length}그룹:`);
  for (const r of nameDup.slice(0, 20)) console.log(`  "${r.norm}"(${r.n}) ids=${r.ids.join(',')} = ${r.names.join(' / ')}`);

  const tot = (await c.query("SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND seed_category NOT LIKE 'bts%'", [cityId])).rows[0].n;
