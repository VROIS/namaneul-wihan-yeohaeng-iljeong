...
(async () => {
  ...
  if (!storageKey) { console.error('✗ Storage 키 미존재 = pull 불가'); process.exit(1); }

  // ① Storage list = 도시폴더 파일 목록 (페이지네이션 = 100개 초과 도시 대응)
  const listAll = async (prefix: string): Promise<string[]> => {
    const all: string[] = [];
    for (let off = 0; ; off += 100) {
      const r = await fetch(`${supaPublicUrl}/storage/v1/object/list/${BUCKET}`, {
        method: 'POST',
  ...
  console.log(`═══ raw-local-pull (city ${cityId}) = ${BUCKET}/${prefix} .json ${files.length}개 → docs/raw/${cityId}/ ═══`);
  if (!files.length) { console.log('  (Storage 에 이 도시 raw 없음 = 당길 것 없음)'); return; }
  ...
  for (const name of files) {
    try {
      const r = await fetch(`${supaPublicUrl}/storage/v1/object/${BUCKET}/${prefix}${encodeURIComponent(name)}`, {
        headers: { Authorization: `Bearer ${storageKey}` },
        signal: AbortSignal.timeout(30000),
      });
      if (!r.ok) { err++; console.log(`  ✗ ${name}: GET ${r.status}`); continue; }
      const buf = Buffer.from(await r.arrayBuffer());
      const localPath = path.join(localDir, path.basename(name));
      // 이미 있고 내용 동일 = skip(중복 write 0). 다르거나 없으면 = write.
      if (fs.existsSync(localPath) && fs.readFileSync(localPath).equals(buf)) { skip++; continue; }
      fs.writeFileSync(localPath, buf);
      ok++; bytes += buf.length;
    } catch (e: any) { err++; console.log(`  ✗ ${name}: ${e.message}`); }
  }
  console.log(`\n═══ 결과 = 신규/갱신 ${ok} / 동일skip ${skip} / 실패 ${err} / ${(bytes / 1024 / 1024).toFixed(2)}MB → docs/raw/${cityId}/ ═══`);
  console.log(`   = 사장님 파일탐색기 docs/raw/${cityId}/ 열람 가능 (= Storage 저장 확인 신호).`);
})();
