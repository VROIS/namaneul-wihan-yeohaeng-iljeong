...
(async () => {
  ...
  console.log(`=== ${city?.name_en || '?'}(${cityId}) 현재 상태 (직접접속) ===\n`);
  ...
  const cat = (await c.query(`
    ...
    GROUP BY seed_category ORDER BY seed_category`, [cityId])).rows;
  console.log(['cat'.padEnd(12), 'n'.padStart(4), 'pid'.padStart(5), 'coord'.padStart(6), 'rc'.padStart(4), 'price'.padStart(6), 'img'.padStart(5), 'sumko'.padStart(6)].join(' '));
  let tot = 0;
  for (const r of cat) {
    tot += Number(r.n);
    console.log([String(r.cat).padEnd(12), String(r.n).padStart(4), String(r.pid).padStart(5), String(r.coord).padStart(6), String(r.rc).padStart(4), String(r.price).padStart(6), String(r.img).padStart(5), String(r.sumko).padStart(6)].join(' '));
  }
  ...
  const band = (await c.query(`
    ...
    FROM place_seed_raw WHERE city_id=$1 AND seed_category='restaurant' AND price_eur IS NOT NULL GROUP BY 1 ORDER BY 1`, [cityId])).rows;
  console.log('식당 band:', band.map((b: any) => `${b.band} ${b.n}`).join(' / ') || '(식당 가격 없음)');
  ...
  const def = (await c.query(`
    ...
    FROM place_seed_raw WHERE city_id=$1 AND seed_category NOT LIKE 'bts%'`, [cityId])).rows[0];
  console.log(`결손: PID없음 ${def.no_pid} / 이미지없음 ${def.no_img} / RC없음 ${def.no_rc} / 가격없음 ${def.no_price}`);
  await c.end();
})();
