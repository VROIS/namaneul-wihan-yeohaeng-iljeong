...
const QUOTA: Record<string, number> = { Economic: 4, Reasonable: 4, Premium: 2, unknown: 2 };
...
(async () => {
  ...
  const { MEAL_BUDGET } = await import(pathToFileURL(path.join(ROOT, 'server/services/agents/types.ts')).href);
  const tierOf = (p: number | null): string => {
    if (p == null) return 'unknown';
    // ⚠️ 사용자 SSOT 2026-06-02 = Luxury(끼당 €181+) → Premium 통합 (= 같은 고가 등급 = 일한도 €300)
    for (const [style, b] of Object.entries(MEAL_BUDGET) as any) if (p >= b.min && p <= b.max) return style === 'Luxury' ? 'Premium' : style;
    return 'unknown';
  };
  ...
  const internalDups = Object.entries(pidGroup).filter(([, a]) => (a as any[]).length > 1);
  const noCoords = kept.filter((p) => !(p.lat && p.lng));
  const noPrice = kept.filter((p) => p.price_eur == null);
  console.log(`\n=== 사전 조사 ===`);
  console.log(`① 내부 중복 (같은 place_id가 2+ 명소에) = ${internalDups.length}건`);
  internalDups.forEach(([, a]) => console.log(`   ⚠️ "${(a as any[])[0].name}" = ${(a as any[]).map((x) => x.zone).join(' + ')}`));
  console.log(`② 입지(좌표) 완비 = ${kept.length - noCoords.length}/${kept.length}${noCoords.length ? ' ⚠️ 누락 ' + noCoords.map((p) => p.name).join(',') : ' ✓'}`);
  console.log(`③ 가격 완비 = ${kept.length - noPrice.length}/${kept.length} (unknown ${noPrice.length}${noPrice.length ? ' = ' + noPrice.map((p) => p.name).join(', ') : ''})`);
  console.log(`④ DB 중복 → UPDATE(통합) 예정 ${updP.length}곳 (= upsertPlace가 기존 행에 덮어쓰기, 신규 INSERT 아님):`);
  updP.forEach((p) => console.log(`   - TS "${p.name}"(€${p.price_eur ?? '?'}) → 기존 id=${p._match.id} "${p._match.name_en}" [${p._match.pid ? 'PID매칭' : (p._match.lat ? '좌표/이름매칭' : '?')}]`));
  ...
  if (argv['list'] === 'true') {
    ...
    for (const [z, arr] of Object.entries(byZone)) {
      ...
      for (const t of ['Economic', 'Reasonable', 'Premium', 'unknown']) {
        const ts = (arr as any[]).filter((p) => p.tier === t).sort((a, b) => (b.review_count || 0) - (a.review_count || 0));
        if (ts.length) console.log(`  ${t}: ${ts.map((p) => `${p.name}(${p.review_count}/€${p.price_eur ?? '?'}/${p._match ? 'U' : 'I'})`).join(' · ')}`);
      }
    }
  }
  ...
  async function pmUpload(photoName: string, key: string): Promise<string | null> {
    ...
    try {
      ...
      const ur = await fetch(`https://${SUPA_PROJECT}.supabase.co/storage/v1/object/place-photos/${fp}`, {
        method: 'POST', headers: { 'Authorization': `Bearer ${STORAGE_KEY}`, 'Content-Type': 'image/jpeg', 'x-upsert': 'true' }, body: buf,
      });
      return ur.ok ? `https://${SUPA_PROJECT}.supabase.co/storage/v1/object/public/place-photos/${fp}` : null;
    } catch { return null; }
  }
