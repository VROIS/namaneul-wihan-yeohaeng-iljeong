...
(async () => {
  ...
  const PLACES_KEY = downloadPhoto
    ...
    : null;
  const SUPA_PROJECT =
    (process.env.SUPA_URL || process.env.SUPABASE_PUBLIC_URL || "").match(
      /https:\/\/([^.]+)\.supabase\.co/,
    )?.[1] || "wxebceflvuythuodemro";
  const STORAGE_KEY =
  ...
  async function pmUpload(
    ...
    try {
      ...
      const pr = await fetch(
        `https://places.googleapis.com/v1/${photoName}/media?maxWidthPx=400&key=${PLACES_KEY}`,
      ...
      const ur = await fetch(
        `https://${SUPA_PROJECT}.supabase.co/storage/v1/object/place-photos/${fp}`,
        {
          method: "POST",
          headers: {
      ...
      return ur.ok
        ? `https://${SUPA_PROJECT}.supabase.co/storage/v1/object/public/place-photos/${fp}`
        : null;
    } catch {
      return null;
    }
  ...
  for (const p of kept) {
    ...
    if (downloadPhoto && p._pmNeed && p.photo_name) {
      const u = await pmUpload(p.photo_name, p.place_id);
      if (u) {
        imageUrl = u;
        photoOk++;
      }
