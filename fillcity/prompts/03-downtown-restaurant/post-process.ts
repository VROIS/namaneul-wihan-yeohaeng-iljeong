...
(async () => {
  ...
  for (const tier of tiers) {
    ...
    const latest = latestVersioned(
      rawDir,
      rawName(3, "downtown-restaurant", tier, date),
    );
    const p = latest
      ? path.join(rawDir, latest)
      : path.join(rawDir, rawName(3, "downtown-restaurant", tier, date)); // ⚠️ 수정금지(승인필요) — raw 파일명 표준화: 날짜앞 rawName 형식
    if (!fs.existsSync(p)) {
  ...
  ); // ⚠️ 2026-06-08 = Windows ESM file:// 변환 (ERR_UNSUPPORTED_ESM_URL_SCHEME 수정)
