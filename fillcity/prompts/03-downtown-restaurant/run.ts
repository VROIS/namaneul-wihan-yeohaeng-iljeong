...
const TIER_SPECS = {
  economic: {
    label: "30 ECONOMIC",
    spec: "30 ECONOMIC (= 1인당 평균 ~€24 이하 = 베이커리/크레페리/패스트/한식 분식)",
  },
  reasonable: {
    label: "30 REASONABLE",
  ...
  premium: {
    label: "30 PREMIUM",
    spec: "30 PREMIUM (= 1인당 평균 €61-180 = 미슐랭 빕구르망/한국 vlog 인기 다이닝)",
  },
  luxury: {
    label: "30 LUXURY",
    spec: "30 LUXURY (= 1인당 평균 €181+ = 미슐랭 1+ 스타/시그너처)",
  },
};
...
(async () => {
  ...
  function build(
    tier: "economic" | "reasonable" | "premium" | "luxury",
    excludeList = "",
  ): string {
  ...
  for (const tier of ["economic", "reasonable", "premium", "luxury"] as const) {
    ...
    const outPath = path.join(
      ...
      versionedName(
        outDir,
        rawName(3, "downtown-restaurant", tier, today),
        rawHash(r.text),
        hashOf,
      ),
    );
  ...
  console.log(
    `  npx tsx fillcity/prompts/03-downtown-restaurant/post-process.ts --city-id=${cityId} --date=${today}`,
  );
})();
