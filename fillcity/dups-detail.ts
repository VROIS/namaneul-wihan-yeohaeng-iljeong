...
const FILL_COLS = [
  'name_ko', 'name_en', 'name_local', 'image_url', 'image_attribution', 'price_eur',
  'editorial_summary', 'summary_ko', 'address', 'latitude', 'longitude',
  'google_review_count', 'google_rating', 'google_maps_uri', 'google_primary_type',
  'opening_hours', 'vibe_keywords', 'phase_tags', 'category_tags', 'names_i18n', 'photo_urls',
  'distance_km_from_center', 'day_zone',
];
...
(async () => {
  ...
  await c.connect();
  console.log(`=== city(${cityId}) PID 중복그룹 keep/loser 필드 비교 (직접접속, 조사전용) ===\n`);
  ...
  const groups = (await c.query(`
    ...
    GROUP BY google_place_id HAVING COUNT(*) > 1 ORDER BY MIN(id)`, [cityId])).rows;

  let cleanDelete = 0, needAbsorb = 0;
  const summary: any[] = [];

  for (const g of groups) {
    const ids: number[] = g.ids;
    const rows = (await c.query(
      `SELECT * FROM place_seed_raw WHERE id = ANY($1::int[])`, [ids]
    )).rows;
    ...
    const sorted = [...rows].sort((a, b) => {
      const rcA = a.google_review_count ?? -1, rcB = b.google_review_count ?? -1;
    ...
    const klass = absorbFields.length === 0 ? 'A_DELETE_ONLY' : 'B_ABSORB_THEN_DELETE';
    if (absorbFields.length === 0) cleanDelete++; else needAbsorb++;

    console.log(`PID=${g.pid}`);
    console.log(`  cat=${keep.seed_category} name="${keep.name_en}"`);
    console.log(`  KEEP id=${keep.id} (RC=${keep.google_review_count ?? 'null'} upd=${keep.updated_at ? new Date(keep.updated_at).toISOString().slice(0,10) : 'null'})`);
    for (const l of losers) {
      console.log(`  LOSER id=${l.id} (RC=${l.google_review_count ?? 'null'} upd=${l.updated_at ? new Date(l.updated_at).toISOString().slice(0,10) : 'null'})`);
    }
    console.log(`  => ${klass}${absorbFields.length ? ' 흡수필요=[' + absorbFields.join(',') + ']' : ' (loser 흡수값 0 = 그냥 DELETE)'}\n`);

    summary.push({ pid: g.pid, name: keep.name_en, keep: keep.id, losers: losers.map(l=>l.id), klass, absorbFields });
  }

  console.log(`\n===== 분류 요약 =====`);
  console.log(`총 PID그룹 = ${groups.length}`);
  console.log(`A) loser 그냥 DELETE 가능 (흡수값 0) = ${cleanDelete}그룹`);
  console.log(`B) 흡수 후 DELETE 필요 (loser 가 keep 빈칸 채움) = ${needAbsorb}그룹`);
  console.log(`\n전체 loser id (DELETE 대상) = ${summary.flatMap(s=>s.losers).join(',')}`);
  if (needAbsorb > 0) {
    console.log(`\nB 그룹 상세 (흡수 필요):`);
    for (const s of summary.filter(x=>x.klass==='B_ABSORB_THEN_DELETE')) {
      console.log(`  keep=${s.keep} <- loser=${s.losers.join(',')} 필드=[${s.absorbFields.join(',')}]`);
    }
  }
  await c.end();
})();
