...
(async () => {
  ...
  log(
    `= DDL/INSERT/UPDATE/DELETE 0 = SELECT·파일저장만 (사장님 승인 전 비파괴)`,
  );
  log(`================================================================\n`);
  ...
  const pidDup = (
    ...
    GROUP BY google_place_id HAVING COUNT(*) > 1 ORDER BY n DESC, ids`,
      [cityId],
    )
  ).rows;
  log(
    `[1-A] PID 중복 = ${pidDup.length}그룹 (같은 google_place_id 2+행 = 명백 중복):`,
  );
  for (const r of pidDup) {
    log(`   PID=${r.pid} (${r.n}행) ids=[${r.ids.join(",")}]`);
    for (let i = 0; i < r.ids.length; i++)
      log(
        `        id=${r.ids[i]} "${r.names[i]}" 중복의심태그=${r.suspect_flags[i] ? "Y" : "N"}`,
      );
  }
  log("");
  ...
  for (const r of suspectRows) {
    log(
      `   id=${r.id} "${r.name_en}" cat=${r.seed_category} 의심대상=[${(r.suspect_targets || []).join(",")}] pid=${r.google_place_id || "-"}`,
    );
  }
  log("");
  ...
  for (const r of coordDup) {
    log(
      `   id ${r.id_a} <-> ${r.id_b} : "${r.name_a}" <-> "${r.name_b}" @${r.lat_a},${r.lng_a} pid=${r.pid_a || "-"}/${r.pid_b || "-"}`,
    );
  }
  log("");
  ...
  const blockedByA = (
    ...
    ORDER BY a.google_place_id, a.id`,
      [cityId],
    )
  ).rows;
  const blockedDistinctRows = new Set(blockedByA.map((r: any) => r.id)).size;
  log(
    `[2-(a)] A(트리거 UPDATE 확장) 적용 시 막혔을 행 = ${blockedDistinctRows}행 (= B 대응 필요량):`,
  );
  for (const r of blockedByA)
    log(
      `   id=${r.id} "${r.name_en}" pid=${r.pid} (다른행 id=${r.other_id} 에 동일 PID 존재)`,
    );
  log(
    `   => B(#45 PID 선검사 = 같은 city 다른 행에 PID 있으면 upsertPlace 병합+현재행 스킵) 대응 필요 = ${blockedDistinctRows}행.`,
  );
  log(
  ...
  const selfCoord = (
    await c.query(
      "SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND latitude IS NOT NULL AND longitude IS NOT NULL",
      [cityId],
    )
  ).rows[0].n;
  const selfUri = (
    await c.query(
      "SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND google_maps_uri IS NOT NULL AND google_maps_uri <> ''",
      [cityId],
    )
  ).rows[0].n;
  const selfLocal = (
    await c.query(
      "SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND TRIM(COALESCE(name_local,'')) <> ''",
      [cityId],
    )
  ).rows[0].n;
  const selfAddr = (
    await c.query(
      "SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND LENGTH(TRIM(COALESCE(address,''))) >= 20",
      [cityId],
    )
  ).rows[0].n;
  const anySelf = (
    await c.query(
      `SELECT COUNT(*)::int AS n FROM place_seed_raw WHERE city_id=$1 AND (
      ...
      (LENGTH(TRIM(COALESCE(address,''))) >= 20) )`,
      [cityId],
    )
  ).rows[0].n;
  log(
    `[2-(b)] 자기행 제외(AND c.id <> COALESCE(NEW.id,-1)) 누락 시 마비 위험 = 자기 자신과 충돌:`,
  );
  log(
    `   PID 보유행 = ${selfPid} / URI 보유행 = ${selfUri} / 좌표 보유행 = ${selfCoord} / 로컬이름 보유행 = ${selfLocal} / 주소20+ 보유행 = ${selfAddr}`,
  );
  log(
    `   => 위 불변검사 중 하나라도 걸리는 행 = ${anySelf}행 (= 전체 ${totAll} 중) = 자기행 제외 없으면 UPDATE 시 100% 자기 자신에 막힘 = 전수 마비.`,
  );
  log(
  ...
  log(
    `   - 불변1 PID: SELECT ... WHERE city_id=NEW.city_id AND google_place_id=NEW.pid AND c.id <> COALESCE(NEW.id,-1)`,
  );
  log(
    `   - 자기 자신(id=X)은 c.id <> NEW.id(=X) 로 제외 => 자기 PID 갱신은 안 막힘 (정상 통과). [OK 논리]`,
  );
  log(
    `   - 단, 같은 PID 를 가진 '다른' 행(id=Y)이 이미 있으면 => 막힘 = 이게 [2-(a)] 의 ${blockedDistinctRows}행 = B 가 선검사로 병합 처리해야 함.`,
  );
  log(
  ...
  const trigBind =
    (
      await c.query(
        `SELECT pg_get_triggerdef(oid) AS def FROM pg_trigger WHERE tgname='place_seed_raw_prevent_dup_trigger' AND tgrelid='public.place_seed_raw'::regclass`,
      )
    ).rows[0]?.def || "(트리거 바인딩 조회 실패)";
