...
(async () => {
  ...
  if (fnStart < 0) { console.error('X 함수 정의 시작점 미발견'); process.exit(1); }
  ...
  const t = (await c.query(`
    ...
    FROM pg_trigger WHERE tgrelid='public.place_seed_raw'::regclass AND tgname='place_seed_raw_prevent_dup_trigger' AND NOT tgisinternal`)).rows[0];
