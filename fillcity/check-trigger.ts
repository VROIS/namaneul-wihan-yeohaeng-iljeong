...
(async () => {
  ...
  for (const t of trigs) console.log(`  ${t.name} = ${t.timing} ${t.events} -> ${t.func}() [${t.enabled === 'O' ? '활성' : '상태=' + t.enabled}]`);

  console.log('\n[2] 트리거 함수 본문:');
  for (const t of trigs) {
    const src = (await c.query('SELECT pg_get_functiondef($1::regproc) AS def', [t.func])).rows[0]?.def || '';
    console.log(`\n----- ${t.func}() -----`);
