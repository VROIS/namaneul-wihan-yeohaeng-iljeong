...
## 📌 정본문서 — 작업 시작 전 이 파일 다음으로 반드시 먼저 읽을 것 (2026-08-25 사장님 확정)
...
> **작업 시작 = CLAUDE.md(이 파일) → 정본문서 → 그다음 필요한 메모리/코드 순으로 확인.** 정본문서 확인 없이 "뭐부터 할까요" 질문 금지 — 거기 다 있다.

**현재 정본 = [`docs/2026-08-25 Tripis v1 안정화.md`](docs/2026-08-25%20Tripis%20v1%20안정화.md)**
...
- 작업 정리·다음 할 일 = 이 파일 맨 위 "📌 정본문서" 링크(현재 = `docs/2026-08-25 Tripis v1 안정화.md`) 1벌. `docs/WORKLOG.md`는 2026-08-25부로 신규 기록 중단, 아카이브 전용.
...
3. **커밋 훅(pre-commit) = 토큰(§10 스탬프, 맨 앞) → 가드3 상시 → 지문 대조(기계 불일치 = 그 자리 1회 재실행 / 코드 커밋인데 실증·판단 마커 없음·낡음 = 커밋 물리 차단, 문서 전용 커밋만 면제) → needs-rebuild(빌드필요) 자동 표시.** post-commit = 토큰·마커 1회용 소멸.
