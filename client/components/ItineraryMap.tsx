...
import { COLORS as SSOT_COLORS, LUCIDE as SSOT_LUCIDE } from "./bts/bts-marker-svg";
...
function makeWebIcon(google: any, cat: string, isStart: boolean, slot: number | null, isSelected = false) {
  ...
  if (!isStart && slot) {
    badge =
      '<g transform="translate(' + (size - 9) + ',9)"><circle r="8" fill="white" stroke="' +
      color + '" stroke-width="1.5"/><text x="0" y="3" text-anchor="middle" font-family="Arial, sans-serif" font-size="9" font-weight="bold" fill="' +
      color + '">' + slot + "</text></g>";
  }
  const svg =
    '<svg xmlns="http://www.w3.org/2000/svg" width="' + size + '" height="' + size + '" viewBox="0 0 ' + size + " " + size + '">' +
    '<circle cx="' + size / 2 + '" cy="' + size / 2 + '" r="' + (size / 2 - 2) + '" fill="' + color + '" stroke="white" stroke-width="3"/>' +
    '<g transform="translate(' + off + "," + off + ") scale(" + sc + ')" fill="none" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">' +
    path + "</g>" + badge + "</svg>";
  return {
...
function ItineraryMapWeb({ places, start, onMarkerPress, selectedSlotId, height = 240, tint = "#2563eb", apiKey }: Props & { apiKey: string }) {
  ...
  useEffect(() => {
    ...
    let count = 0;
    if (startMarkerRef.current) { b.extend(startMarkerRef.current.getPosition()); count++; }
    for (const id of Object.keys(markersRef.current)) { b.extend(markersRef.current[id].getPosition()); count++; }
...
function ItineraryMapNative({ places, start, onMarkerPress, selectedSlotId, height = 240, tint = "#2563eb", apiKey }: Props & { apiKey: string }) {
  ...
  const [mapReady, setMapReady] = useState(false);
  const html = useMemo(() => (apiKey ? ITINERARY_MAP_HTML(apiKey) : ""), [apiKey]);
  ...
  function onMessage(e: any) {
    try {
      ...
      else if (data.type === "marker" && typeof data.id === "string") onMarkerPress(data.id);
      else if (data.type === "error") console.warn("[ItineraryMap] WebView error:", data.message);
    } catch {}
  }
...
export default function ItineraryMap(props: Props) {
  ...
  if (Platform.OS === "web") return <ItineraryMapWeb {...props} apiKey={apiKey} />;
