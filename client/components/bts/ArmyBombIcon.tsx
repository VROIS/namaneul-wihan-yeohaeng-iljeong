...
export function ArmyBombIcon({ size = 24, color = BORAHAE, glowing = false }: ArmyBombIconProps) {
    ...
    <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <Defs>
        ...
        <RadialGradient id="globeGlow" cx="50%" cy="40%" r="50%">
          <Stop offset="0%" stopColor="#FFFFFF" stopOpacity={glowing ? 0.9 : 0.5} />
          <Stop offset="50%" stopColor={color} stopOpacity={glowing ? 0.6 : 0.3} />
          <Stop offset="100%" stopColor={color} stopOpacity={glowing ? 0.3 : 0.1} />
        </RadialGradient>
        {/* 글로우 후광 */}
        {glowing && (
      ...
      <G>
        {/* 글로우 후광 (glowing일 때만) */}
        {glowing && (
          <Circle cx={cx} cy={globeCy} r={globeR * 1.8} fill="url(#outerGlow)" />
        )}

        {/* 구체 */}
        <Circle cx={cx} cy={globeCy} r={globeR} fill="url(#globeGlow)" />
        <Circle cx={cx} cy={globeCy} r={globeR} fill="none" stroke={color} strokeWidth={0.5} opacity={0.5} />
