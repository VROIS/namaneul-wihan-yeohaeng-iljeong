...
export function PlaceAutocomplete({
  ...
  const handleSelect = async (prediction: Prediction) => {
    ...
    try {
      const response = await apiRequest(
        "GET",
        `/api/places/details?placeId=${prediction.placeId}`,
      );
  ...
  return (
    <View style={[styles.container, { zIndex }]}>
      <View
        style={[styles.inputBox, { backgroundColor: theme.backgroundDefault }]}
      >
        <Icon
          name={icon}
          size={20}
          color={isSelected ? Brand.primary : theme.textTertiary}
        />
        <TextInput
        ...
        {isSelected && query.length > 0 && (
          <Pressable onPress={handleClear} hitSlop={8}>
            <Icon name="x-circle" size={18} color={theme.textTertiary} />
          </Pressable>
        )}
      </View>
      ...
      {showDropdown && predictions.length > 0 && (
        <View
          style={[
            styles.dropdown,
            { backgroundColor: theme.background, zIndex: zIndex + 1 },
          ]}
        >
          <FlatList
            data={predictions}
