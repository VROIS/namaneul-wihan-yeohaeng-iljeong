...
import { pgTable, varchar, integer, text, timestamp, boolean, jsonb } from "drizzle-orm/pg-core";
...
export const expertInquiries = pgTable("expert_inquiries", {
  ...
  itineraryId: integer("itinerary_id").references(() => itineraries.id, { onDelete: 'set null' }), // 저장 여정이면 연결(미저장 즉석 여정은 null)
  ...
  expertId: varchar("expert_id").references(() => users.id, { onDelete: 'set null' }), // 답변한 전문가(다수 대비). SET NULL = 전문가 탈퇴해도 답변 보존(시뮬 발견 2026-07-13)
...
export const insertExpertInquirySchema = createInsertSchema(expertInquiries).omit({
  id: true, status: true, expertId: true, expertReply: true, isReadByUser: true, createdAt: true, answeredAt: true,
});
