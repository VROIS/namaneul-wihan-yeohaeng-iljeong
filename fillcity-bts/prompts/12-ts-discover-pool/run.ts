...
(async () => {
  ...
  // ⚠️ 2026-08-07 사장님 승인 = BTS 전용 사본 = 공연장(bts_venue) 좌표를 여기서 함께 읽음(= 아래 c.end() 로 연결이 닫히므로 이 구간이 유일한 조회 지점).
  //   = 공연장은 이미 DB 에 있는 행 = 발굴 대상 아님 = 식당 검색의 "기준점"으로만 사용.
  const venueRow =
    zone === "venue"
      ? (
          await c.query(
  ...
  //   downtown(6cat 100km 사각형) = 중심좌표만 필요 = cities 에 있음
  // ⚠️ 2026-08-07 사장님 승인 = BTS 전용 사본 = zone=venue(식당) = 기준점이 "도심 좌표"가 아니라 "공연장(bts_venue) 좌표".
  //   = 공연장은 이미 DB 에 있는 행 = 발굴하는 게 아니라 그 좌표를 기준점으로만 씀. 도심/외곽 구분 없음(반경 1경로).
  let dests = DISCOVERY_ZONES[cityId]?.[zone];
  if (!dests?.length) {
    if (zone === "venue") {
      if (venueRow?.latitude == null || venueRow?.longitude == null) {
        console.error(
          `city ${cityId} bts_venue 좌표 없음 = 공연장 기준 식당 발굴 불가`,
        );
        process.exit(1);
      }
      dests = [
        {
          name: venueRow.name_en || "venue",
          lat: Number(venueRow.latitude),
          lng: Number(venueRow.longitude),
          radius: 50000,
        },
      ];
      console.log(
        `ℹ️ zone=venue → 공연장 좌표 기준 (${venueRow.name_en} ${venueRow.latitude},${venueRow.longitude}, 50km)`,
      );
    } else if (
      zone === "downtown" &&
      city?.latitude != null &&
