...
(async () => {
  // ⚠️ 수정금지(승인필요) 2026-06-23 사장님 SSOT(§3-A) = 진입분기 자동 = --only 미지정 시 비BTS 행수로 갈래 결정.
  //   = BTS 사본 임계 = 30(TOP5×6cat 완주 산식. 원본 120=TOP20×6 동형 조정 = 2026-08-07 §22 판단검증:
  //     120 그대로면 BTS 목표(~50행)로는 도달 불가 = 완료 도시도 재실행마다 유료 6cat 재발굴).
  //   = 행수 ≥30 = 이미 1차 발굴됨 = 변형 갈래 [정제→식당발굴→#45] / <30 = 풀 갈래 [+6cat discover].
  ...
  if (only.includes("restaurant")) {
    // ⚠️ 2026-08-07 사장님 승인 = BTS 전용 사본 = 식당 = 공연장(bts_venue) 좌표 기준 반경 50km 1경로로 통일.
    //   = 옛 도심(03+TS 3종)·외곽(04+town) 2경로 = 도시중심 기준이라 BTS 목적(공연장 접근성)과 안 맞음 → 이 사본에서 제거.
    //   = 일일 여정(점심·저녁 2슬롯)에 앱이 쓰는 식당 후보 상한이 20 이므로(server/bts-routes.ts) 인기순 20 = 1콜로 충분.
    //   = 원본 fillcity/ 는 도심·외곽 2경로 그대로 = 유럽 무영향.
    run(
      `Ⓑ 공연장 반경 식당(50km 인기순)`,
      "prompts/12-ts-discover-pool/run.ts",
      [
        `--city-id=${cityId}`,
        "--zone=venue",
        "--method=nearby",
    ...
    // ⚠️ 수정금지(승인필요) 2026-06-23 사장님 SSOT = 식당발굴은 PM 안 함(--photo 제거). PM(이미지)은 #45 단계에서만.
    run(
      `Ⓑ 공연장 반경 식당 병합`,
      "prompts/12-ts-discover-pool/post-process.ts",
