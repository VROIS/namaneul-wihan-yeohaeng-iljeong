...
| **동일인 통합 (2026-02-24)** | user_providers 테이블, provider 1순위·birth_date 2순위 매칭. migrations/0009_user_providers.sql. Supabase에서 마이그레이션 실행 후 배포. 구글/카카오/페이스북 각각 로그인 시 1명으로 통합. |
...
| **Google OAuth URL (2026-02)** | Google Cloud Console에 넣을 URL: **프로덕션** `https://legal-dannye-dbstour-4e6b86d5.koyeb.app` (JavaScript 원본·리디렉션 URI 둘 다). **로컬** `http://localhost:8082` 또는 `http://192.168.1.23:8082` (query-client.ts fallback). (localhost:19006 미사용) |
| **Google OAuth 클라이언트 ID (2026-02)** | 로컬: `.env.local`. DB: api_keys `GOOGLE_OAUTH_CLIENT_ID`. **Koyeb**: 환경변수 `EXPO_PUBLIC_GOOGLE_CLIENT_ID` 추가 → 빌드·런타임 모두 적용 (Dockerfile ARG 반영). |
