...
| # | 작업 | 상태 | 파일 |
|---|------|------|------|
| 1 | 하단 탭 5개 구성 (일정→지도→전문가→프로필→설정) | ✅ | `MainTabNavigator.tsx` |
| 2 | 지도 탭 = 토글 기능 (MapToggleContext) | ✅ | `MapToggleContext.tsx`, `App.tsx` |
| 3 | 전문가 검증 CTA 버튼 삭제 (하단탭 이동) | ✅ | `TripPlannerScreen.tsx` |
| 4 | 섹션 간격 최소화 (여백 축소) | ✅ | `TripPlannerScreen.tsx` |
| 5 | 요약 섹션 "누구를 위한 X 여행" 문구 변경 | ✅ | `TripPlannerScreen.tsx` |
| 6 | 이동 거리/가격 백엔드 연결 (transit 데이터) | ✅ | `itinerary-generator.ts` |
| 7 | cursor-dev 브랜치 생성 및 GitHub 푸시 | ✅ | Git |
...
## Phase 1: 핵심 기능 (우선순위 상)

### 1️⃣ 여정 생성 + 예산 병렬 처리
- **현재**: 순차 생성 (느림)
- **목표**: 2개 API 병렬 호출 → 속도 향상, 프롬프트 분리
- **파일**: `itinerary-generator.ts`, `budget-calculator.ts`
- **상세**:
  - Promise.all([generatePlaces(), calculateBudget()])
  - 각 프롬프트 세밀화 가능

### 2️⃣ 요약 섹션 예상 비용 표시 ⭐ 중요
- **현재**: "가족을 위한 힐링과 미식 여행"
- **목표**: "가족을 위한 힐링과 미식 여행 · 예상 €1,250"
- **파일**: `TripPlannerScreen.tsx`
- **주의**: 민감한 정보 → 정확도 필수

### 5️⃣ 교통비 로직 수정 (가이드 비용 비교 표시)
- **조건별 표시**:
  - Luxury/Premium + 이동최소화 → 가이드 비용만 (대중교통 숨김)
  - 그 외 → 대중교통/우버 + "가이드 이용 시 €XX/인" 추천
- **마케팅 포인트**: 모든 사용자에게 가이드 비용 비교 표시 → 신규 고객 진입점
- **파일**: `transport-pricing-service.ts`, `TripPlannerScreen.tsx`

### 6️⃣ 식사 슬롯 필수 포함
- **규칙**: 점심, 저녁은 무조건 슬롯에 포함 (아침 제외)
- **예산**: TravelStyle별 €10/30/50/70 범위에서 AI 선택
- **파일**: `itinerary-generator.ts`

### 10️⃣ 위기 경보 깜박이는 경고 표시
- **조건**: 일정 생성 시 위기 경보 있으면
- **표시**: 상단 깜박이는 경고 아이콘 → 클릭 시 팝업
- **파일**: `TripPlannerScreen.tsx`

---

## Phase 2: UX 개선 (우선순위 중)

### 3️⃣ 감성 데이터 표시 (AI 추천 대체)
- **현재**: "AI가 추천한 장소"
- **목표**: "백종원 언급 | 네이버 블로그 인기 | 인스타 핫플" 등 3가지 요약
- **파일**: `TripPlannerScreen.tsx`, `korean-sentiment-service.ts`

### 4️⃣ 썸네일 이미지 필수 표시
- **소스**: Google Places, Instagram, MediaWiki
- **규칙**: 무조건 표시 (없으면 placeholder)
- **파일**: `itinerary-generator.ts`, `TripPlannerScreen.tsx`

### 5️⃣ NUBI 추천 점수 (별표 + 소숫점)
- **현재**: ⭐⭐⭐⭐⭐ (정수)
- **목표**: NUBI 4.7 ⭐ (구글맵 평점 스타일)
- **파일**: `TripPlannerScreen.tsx`

### 8️⃣ 날씨/위기 경보 일정표 시작에 표시
- **위치**: 일정표 최상단
- **데이터**: OpenWeather API, GDELT/Gemini
- **파일**: `TripPlannerScreen.tsx`

---

## Phase 3: 브랜딩/저장 (우선순위 하)

### 7️⃣ 전문가 검증 마크
- **상태**: 검증됨 / 요청중
- **위치**: 일별 날짜 옆 인증마크
- **전문가 버튼**: 다른 색깔 + 애니메이션 → 클릭 유도
- **파일**: `TripPlannerScreen.tsx`, `MainTabNavigator.tsx`

### 9️⃣ 저장 버튼 구현
- **위치**: 일정표 상단
- **동작**: 프로필에 저장 → 추후 불러오기
- **파일**: `TripPlannerScreen.tsx`, `storage.ts`
...
```
1단계: 식사 슬롯 필수 포함 (#6) → 일정 품질 향상
2단계: 예상 비용 표시 (#2) → 사용자 신뢰도
3단계: 교통비 로직 (#5) → 가이드 마케팅
4단계: 위기 경보 표시 (#10) → 안전/신뢰
5단계: 감성 데이터 (#3) → 차별화
6단계: 썸네일 이미지 (#4) → 시각적 완성도
7단계: NUBI 점수 (#5) → 브랜딩
8단계: 날씨/위기 (#8) → 실용성
9단계: 전문가 마크 (#7) → 신뢰도
10단계: 저장 버튼 (#9) → 사용자 편의
