...
**Gemini 대시보드 실측(사장님 캡처)**: **이미 Tier 2.** 병목 = 텍스트 아님(4/2,000 RPM) = **영상 씬 모델**(Veo 6/4·Omni 9/8 = 한도 초과 이력). 한도 상향 = 요청 폼/Tier 3/정식판 전환. 스토리보드 실측 모델 = `gemini-3-flash-preview`(2.5 Flash 아님 — 전환 여부 사장님 결정 대기).
