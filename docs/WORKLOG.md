# WORKLOG — 단일 통합 작업 일지 (2026-08-25부로 신규 기록 중단, 아카이브 전용)

> ⚠️ **이 문서는 더 이상 갱신하지 않는다(2026-08-25 사장님 확정).** AI가 실제로 안 읽고 기록량만 쌓이던 문제로 폐기.
> **새 작업계획·일지 = 사장님이 옵시디언 "tripis" 볼트(`mhg2자료/tripis`)에 직접 작성 → 정본문서로 기록되어 진행.**
> **현재 정본 = [`docs/2026-08-25 Tripis v1 안정화.md`](2026-08-25%20Tripis%20v1%20안정화.md)** (이후 후속 정본이 생기면 이 링크를 그 문서로 갱신).
> 아래 내용은 2026-08-25 이전 작업 이력 아카이브로 그대로 보존.

> **정책 (= 사용자 SSOT 2026-05-14, 아카이브 기간에 적용됐던 옛 정책)**:
...
## 🎛️ 2026-08-25 관제탑(관리자 대시보드) 스펙 v2 전면 배선 + AI 성능 카드 실활성화 (판단3종 3/3 통과)

**배경**: 옛 관제탑이 죽은 필드(오늘동기화·API연결·캐시데이터 등, BE가 안 주던 값)로 껍데기만 보여주고 있던 것을 사장님이 직접 클릭·조사하며 전수 적발. "완전 껍데기만 흉내만 내는 구현" 질책 이후 전면 재배선.

**한 것(전부 §22 순서: 기계검증→라이브 실증→판단3종 완료, 커밋은 사장님 승인 대기)**:
- **헤더**: 고정요소(로고·시각·TS/PM경고) `flex-shrink:0` + 가변배지(API키 확인필요, 3개+"외 N건")를 맨 뒤로 — 요소 개수 변해도 앞쪽 안 밀리게.
- **수익 카드**: AI 비용(추정) = `UNIT_COST_LEDGER`(공개단가·출처URL·확인일 전부 기록하는 가계부, ts/pm/veo/omni/nano 리서치 재확인) 기준 재계산, **공개단가 대비 +21% 할증**(사장님 실청구 확인) 반영. `UNIT_COST_EUR`는 이 가계부에서 자동 파생(수기 이중관리 안 함). "ARPU"→"1인당 평균매출"(쉬운말).
- **전체 사용자 크레딧 내역**: 충전(+)만 보여주던 것 → 전체 사용자의 +/− 전체 거래를 이메일 식별과 함께 엑셀표로, 상단에 현재 전체 보유크레딧 총합 표시.
- **API 키 관리**: 삭제 버튼이 서버 라우트(`DELETE /api/admin/api-keys/:keyName`, 소프트삭제)는 있었는데 화면에 없던 것 발견 → 버튼+확인창 추가, 목록 조회도 `is_active=false` 제외하도록 정정(삭제해도 화면에 남아있던 버그 동시 수정).
- **AI 성능 카드 실활성화(이번 세션 핵심)**: 내손앱 `api_logs` 표(271건, 2026-03-25에 멈춘 죽은 스냅샷, 되살리면 §16 "카운터 2벌" 위반) 발견 → 되살리지 않고, 지금도 계속 쓰는 유일 카운터 `external_calls`에 `response_time_ms`/`success`/`error_message` 3컬럼만 추가(마이그 0024). 유일한 제미니 진입점 `geminiClient.ts`의 `geminiJson()`·`geminiVisionStream()`(가이드 사진해설 스트림, 그동안 계측 자체가 없었음) 양쪽에 시작~종료 타이머 배선. 최근 100건 집계 함수 `geminiPerformance()` 신설 → `/api/admin/activity-summary`에 포함. **실측 1콜로 라이브 검증**: DB에 `response_time_ms:1290, success:true` 실기록 확인 + 크롬DEV로 `/admin` API상태탭 접속, "AI 성능(최근 1건 기준)" 카드에 1.3초/100%/0% 실제 렌더링 스크린샷 확인.
- **콘텐츠관리(도시카드 선별입력)**: 대표사진·하이라이트3·영상을 관리자가 PSR id/영상 id로 수동 override(`cities.override_*` 3컬럼, 마이그 0023). null=자동랭킹 그대로. 영상 override가 `card.itineraryId`(코스 배지가 쓰는 그 필드)를 덮어써 [코스] 이동이 엉뚱한 여정으로 튀던 버그를 판단3종이 잡아 `videoItineraryId`/`videoDay` 분리로 즉시 수정.
- **+신규 배지**: 30초 자동새로고침마다 기준시각이 매번 밀려 델타가 사실상 0/1만 보이던 것 → `?markViewed=1`을 세션당 최초 1회만.

**판단3종(Workflow, 3/3 통과, blockers 0)**: simplify — 재사용·병렬화 양호(UNIT_COST_EUR 자동파생, 기존 조회 API 재사용). review — 코드경로 끝까지 추적해도 회귀 없음(사소한 주석 오기재 2건만, 기능 무관). react-best — TripisModal의 itineraryId const→state 전환, 모달 마운트 구조상 stale-state 위험 없음 확인.

**중요 확인(사장님 질문 → 실측 답변)**: 로컬호스트와 운영앱(my-guide.replit.app)은 **DB는 공유**(로컬에서 한 마이그레이션·API키 삭제·테스트 호출 전부 진짜 운영 DB에 실기록됨, 직접 SELECT로 증명)하지만 **코드는 분리**(오늘 고친 코드는 전부 미커밋 상태로 로컬에만 있음, `git log` 마지막 커밋 = 8/24 21:12) — 커밋 + 사장님 Replit Publish 전까지 실제 사용자 화면엔 반영 안 됨.

**별건**: Google Play vc26 반려(사진·동영상 권한 정책) + vc27 재제출 완료 = [docs/2026-08-14 실사용자 관점 AAB·IPA 앱버전 최적화.md](2026-08-14%20실사용자%20관점%20AAB·IPA%20앱버전%20최적화.md) "버전 이력 원장" 2026-08-25 행에 기록(중복 방지, 여기선 포인터만).

**커밋·푸시 완료**(사장님 "커밋 푸시" 지시, `316f1eb`, main 푸시 완료). 스탬프 절차 중 Claude Code 자동모드 분류기가 승인토큰 발급 명령을 자체 차단하는 일이 있어 사장님이 수동모드로 잠깐 전환 후 재시도 = 정상 발급·통과(오토모드에서 AI가 이런 자기승인성 명령을 못 돌리는 게 툴 차원의 정상 동작).

**빌드/OTA 필요 여부(훅 자동판정 + 실측)**: IPA·APK 둘 다 불필요(무선 업데이트로 흡수). 서버(마이그레이션 포함)는 Replit Publish 필요(사람이 눌러야 함). 이 푸시가 실제로 발동시킨 EAS Update 워크플로(`32809606511`)를 끝까지 지켜봄 = 3분24초 성공, 무선 업데이트 실제 게시 확인.

**Node 20→22 질문 답변(사장님 질문 → 실측)**: 어제(8/24) 커밋 `73a057a`가 **EAS/앱빌드 GH Actions 워크플로 4개만** 20→22로 올림(이유 = eas-cli 최신판이 Node22 요구, CI 빌드환경 한정). **운영 서버(`.replit`)는 그대로 Node20 유지**(안 건드림, 건드릴 이유도 없음 — `package.json` engines도 "18 이상"뿐이라 20으로 충분, 괜히 운영 런타임 바꾸는 게 더 위험). 방금 그 EAS Update 워크플로가 Node22로 실제 재실행돼 성공한 것으로 CI쪽 수정도 문제없음을 확인.

---

## 💳 2026-08-06 실사용 포렌식 3건 → 크레딧·영상 근본수정 (정본 갱신 = [docs/2026-07-29 결제·크레딧 구현.md](2026-07-29%20결제·크레딧%20구현.md) 머리말)

**사장님 실사용 시나리오 1:1 포렌식**: ①파리 117 = 취소 기능 부재(영상은 완성·게시돼 있었음 = credit-zero-test 계정 소유, 취소 설계 = 보류) ②런던 121 = 실패인데 -60 증발 + 사유 미기록 ③이메일 한 글자 오타(`user~test@`) = 새 계정 + 보너스 50(두 계정 유지, 정식화 때 인증코드 = 체크리스트 등재).

**수정(승인 후 구현, 판단검증 3/3 통과 — 검증이 잡은 회귀 1건 즉시 수정)**:
- **일별영상 차감 = 성공·게시 시점으로 이동**(§9 관문 1벌 확장: `precheckFeature` 사전확인 + `chargeFeature(null,...)` 성공시 차감, 성공 씬 ≥ min(6,요청씬)만 유료). 실패 = 차감 0 = 환불 개념 소멸.
- **실패 사유 기록·표시**: `video_by_day.error` + 화면 "다시 시도" 뭉개기 문구 폐기(서버 사유 그대로).
- **429 재시도 1벌화**: `shared/retry-429.ts` 신설(텍스트 1→2→4→8초), geminiClient(json·vision스트림 생성) 배선, video-gen-client 로컬 사본 삭제(실증 딜레이 유지).
- **관리자 전체 상황판**: 전문가 문의함 role 패턴을 영상·해설·여정 3곳 복제(`getRoleFromDb` 1벌 = auth-user, 판정 = Bearer 토큰만 = 스푸핑 차단).
- 포렌식 도구 `fill/storage-observe.ts`(R2↔SP↔guides 시간대별 관찰) 영구 등재.

**Gemini 대시보드 실측(사장님 캡처)**: **이미 Tier 2.** 병목 = 텍스트 아님(4/2,000 RPM) = **영상 씬 모델**(Veo 6/4·Omni 9/8 = 한도 초과 이력). 한도 상향 = 요청 폼/Tier 3/정식판 전환.

**스토리보드 모델 A/B 시뮬(사장님 방식 = 영상 없이 텍스트 2콜, 여정 121)**: A=현행 3-flash-preview = 6/6 완벽(대사 전부 DB 원문). B=2.5-flash = **씬2 대사 통째 누락 + 프롬프트 장황·태그 남발 + 더 느림(6.5s vs 5.8s)** → **전환 안 함 확정**(한도 실익도 없음 = 텍스트 풀 여유 500배). 스토리보드에 model 통로만 개설(기본 무변경).

**커밋 승인 스탬프 장치 복원(사장님 적발)**: 7-25 훅 이사(.git/hooks→.githooks) 때 스탬프 검사·가드가 유실돼 12일간 무방비였음 → `guard-commit-approval.mjs`(이력 원본) + pre-commit check + post-commit consume 복원, 차단 실증(exit 1). 승인 = "커밋해" 명시 문구뿐("진행해" ≠ 승인 = 메모리 등재).

**로컬 실증(§22 순서 준수, localhost:5057 신 코드 서버 + 실계정 + DB 교차검증)**: ①잔액 20 계정 → 402 + 장부 차감 0 ②관리자 상황판 = 영상 12/12·해설 92/92·여정 75/75(전체 일치)·사용자는 자기 것만 ③429 유닛 = 2회 대기 후 성공·비429 즉시 재던짐. 함정 1건 자체 적발 = 초기 실증이 옛 서버(5000)·Metro(8082)에 호출되고 있었음 → 실증 대상 확인 절차화. 운영 재실증 = Republish 후.

**§22 전과정 기계 자동화(2026-08-06 사장님 확정 순서 지시)**: 확정 순서 = 수정→기계검증4→크롬DEV 실증(사용자 관점)→무한루프→판단3종→표→문서 업뎁→대기→커밋 승인(스탬프)→빌드필요 체크→커밋·푸시.
- 신규 `scripts/verify-pipeline.mjs` = 오케스트레이터 1벌: 각 단계 통과 시 **코드 지문**(코드 경로 한정 staged blob 해시 = 문서 갱신은 지문 불변 → "판단 → 문서 업뎁" 확정 순서 성립) 마커 기록(.verify-state.json) → pre-commit 은 재실행 대신 지문 대조 = **기계검증 중복(4회→1회) 제거**(사장님 지적). 코드 바뀌면 마커 자동 "낡음" = 그 단계 재요구. 코드 커밋인데 실증·판단 마커 없음 = 커밋 물리 차단. 훅 순서 = 토큰(스탬프) 맨 앞 → 가드3 상시(문서 박제 검사 담당) → 지문 대조 → needs-rebuild(빌드필요) 자동 표시. post-commit = 토큰+마커 동시 소멸(1회용).
- 판단3종이 이 장치 자체에서 치명 3건을 사전 적발·수정: ①verify-pipeline 이 .gitignore `scripts/verify-*.mjs` 에 걸려 레포 미추적(=7-25 훅 유실 사고 재생산 위기) → `!` 예외 2건 ②지문 = diff 텍스트 md5 → 1MB 초과 시 게이트 크래시 → 코드 경로 한정 blob 해시로 교체 + maxBuffer 64MB ③코드 경로 판정에 plugins/·App.js·web/·bts-app/·migrations/ 누락 보강. 재검 = 전부 통과.
- 실증 = 문서만 수정 시 지문 불변(ee26446a 동일) + hook-check 2단 차단 + 마커 낡음 자동 표시. ⚠️ 실증 뒷정리 중 checkout 으로 미커밋 WORKLOG 문단 1개 소실 → 즉시 재작성 복구(원인규명 전 삭제 금지 원칙 재확인).
- 잔여 = CLAUDE.md §22 문구를 이 확정 순서로 개정(헌법 = 사장님 승인 대기).

---

## 🚀 2026-08-06 Cloudflare 이전 실행계획 착수 (정본 [docs/2026-08-06 Cloudflare 이전 실행계획.md](2026-08-06%20Cloudflare%20이전%20실행계획.md))

**목표** = Replit €100/월 탈출($5/월) + 창고 위기(0.9/1GB) 해소 + 앱 속도 + 클로드 완전관리 인프라. **사장님 확정** = ①창고(R2) 우선 ②단계별(빅뱅 금지) ③**DB는 포스트그레스 유지(Supabase)** — D1은 다른 계열(SQLite)이라 §14 트리거 재개발 위험으로 제외 ④도메인은 4단계에서 검토.

- **단계 1 = 당일 사실상 완료**(잔여 = 1-5b 소크 후 철거뿐): ①신규 쓰기 R2 전환 6벌+신규 가이드 기기사진 유입구 마감(옛 Supabase PUT·base64 직저장 삭제 §19) ②기존 4,362개 958MB 복사(실패 0, 멱등 교차검증) ③DB 주소 2,606행 치환(승인 후 집행, §14 트리거 = 정식 면제 경로로 통과) ④guides base64 36행 추출(실측 6.3MB — 옛 33MB 기록은 7-30 수치) ⑤운영 실증 = 사장님 조작 + AI 판독 = 사진 27건 r2.dev 200·영상 206·supabase 요청 0·에러 0. R2 대시보드 실측 = 1.02GB/10GB.
- 검증 = 기계 3종(tsc·서버빌드·lint) 통과 + 판단 2종(simplify·review) 워크플로 + needs-rebuild = 오늘 변경은 재빌드 불필요(서버만 = Republish 충분). **커밋 = 지시 대기(§10), 대상 = 수정 14 + 신규 4 파일.**
- 단계 2 = CF 준비(Workers Paid·토큰·Builds 연결). 단계 3 = DB 다이어트(실측 표 서명 후). 단계 4 = 서버 Worker 이사(사전등록 6항 후 컷오버).

---

## 📌 2026-08-02 도시 카드 [해설] 배지 = 각 카드 대표장소 번호 (사장님 직접 테스트용 기록)

사장님이 [업로드] → [장소번호] 로 직접 만들어 보시고 피드백 주실 목록. **AI 가 대신 채우지 않는다(사장님 지시).**

| 도시 | 장소번호 | 장소 | 해설 배지(2026-08-02 기준) |
|---|---|---|---|
| 파리 | **61963** | 루브르 박물관 | 켜짐 |
| 런던 | **62339** | 버킹엄 궁전 | 꺼짐 |
| 마드리드 | **62141** | 레티로 공원 | 꺼짐 |
| 뮌헨 | **67842** | 뮌헨 올림픽 공원 | 꺼짐 |
| 브뤼셀 | **62222** | 오줌싸개 동상 | 꺼짐 |

- 장소번호를 정하는 규칙 = `server/city-place-routes.ts` 의 도심 상위 3곳 조회 1벌 = **그 도시 구글 리뷰 1위**(사진 있음 + 리뷰수 있음 + 식당 아님 + 도심). 그 1등이 카드 사진·한 줄 카피·장소번호를 전부 담당한다.
- 배지는 **그 장소 + 그 언어** 해설이 창고(`guides.place_id` + `language`)에 있으면 자동으로 켜진다 = 대표를 손으로 고르는 칸·절차 없음(2026-08-02 사장님 확정).
- 그래서 **카를 광장(72641) 해설로는 뮌헨 배지가 안 켜진다** — 뮌헨 카드가 가리키는 장소는 올림픽 공원(67842)이기 때문. 정상 동작.
