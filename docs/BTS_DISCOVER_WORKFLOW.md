...
**서브 에이전트 호출 spec** (단일 general-purpose):
...
장소명(한국어·영문), 좌표 (위도/경도), 위 세 조건 근거·출처를 표로 정리해줘."

규칙:
1. 테마 = 한 단어만 (세부 분류 X = 사용자 SSOT)
2. 반경 100km (도시 중심 좌표는 cities 테이블에서 SELECT)
3. 3 조건만 (평점/인스타/매체순위 X)
4. WebSearch 도구 필수 사용
5. 좌표 = Google Maps 또는 Wikipedia 에서 검증 추출 (LLM 추정 X)
...
[
  {
    ...
    "nameKo": "...",
    "nameEn": "...",
    "latitude": 31.7587,        // ⭐ 발굴 시점에 좌표 같이 받기
    "longitude": -106.487,
    "googleSearchNote": "...",
    "googleReviewCountNote": "...",
    "googleImageCountNote": "...",
    "evidenceUrl": "..."
  }, ...
]

저장: scripts/{city}-{category}-30.json
시간: 4~6 분 안에 완료.
```

**좌표 검증 의무 (서브 에이전트)**:
- 각 row 의 좌표 = WebSearch 또는 WebFetch 로 Google Maps URL 확인
- LLM 추정 좌표 = 거부 (메모리 `feedback_llm_hallucination_metadata.md`)
- 좌표 없는 row = `latitude: null, longitude: null` 명시 (Step 2 cron 이 채움)
