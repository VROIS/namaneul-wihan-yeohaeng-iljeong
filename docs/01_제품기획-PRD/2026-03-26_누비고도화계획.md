# 구현 계획 — 6대 슬롯 필수요소 + 5대 가격원칙

> **IMPORTANT**: 코드 수정 전 사용자 승인 필수

---

## A. 슬롯 6대 필수요소 (2026 적용)

| # | 필수요소 | 우선순위 |
|---|----------|----------|
| 1 | 장소명 | name + nameKo |
| 2 | 이미지 1장+ | 셀럽인스타 → 인스타 → 구글 |
| 3 | 선정이유 (nubiReason) | place_nubi_reasons → place_seed_raw → 실시간 검색 |
| 4 | 가격정보 | place_prices → place_seed_raw.priceEur → Gemini |
| 5 | 구글맵 링크 | googleMapsUrl |
| 6 | **1차 요약 (geminiReason)** | Gemini reason → place_seed_raw fallback |

**1차 Gemini 응답 최우선**: 6개 요소가 모두 있으면 그대로 사용. 미완비 시 place_seed_raw fallback.

---

## A-1. 슬롯 5대 필수요소 (DB 감사 결과, 이전)
...
1. 1순위: 셀럽 인스타 이미지 (celebrityImageMap)
2. 2순위: 인스타 사진 (instagramPhotoUrls[0])
3. 3순위: 구글 사진 (photoUrls[0])
4. 4순위: API 사진 (fallback)
...
- **파리 3일 일정** → 트로카데로(€0), 루브르(€32↑), 식당(priceLevel 반영)
- **모든 슬롯**: name✅ image✅ nubiReason✅ price✅ googleMapsUrl✅
