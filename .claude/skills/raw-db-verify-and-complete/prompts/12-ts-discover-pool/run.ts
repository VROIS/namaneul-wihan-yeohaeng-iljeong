...
const label = argv['label'] ? String(argv['label']) : '';
const priceLevels = argv['price-levels'] ? String(argv['price-levels']).split(',').map((s) => `PRICE_LEVEL_${s.trim().toUpperCase()}`) : null;
if (!cityId) { console.error('Usage: --city-id=<N> [--zone=outskirt|downtown] [--per=20] [--pages=1] [--method=text|nearby] [--label=x] [--price-levels=EXPENSIVE,VERY_EXPENSIVE]'); process.exit(1); }
...
(async () => {
  ...
  console.log(`═══ ts-discover-pool (city=${cityId} ${city?.name_en}, zone=${zone}, method=${method}${label ? ' label=' + label : ''}${priceLevels ? ' price=' + priceLevels.join('+') : ''}, ${dests.length} 명소 × ${callsPerDest}) ═══`);
  ...
  for (const d of dests) {
    ...
    do {
      ...
      const body: any = isNearby ? {
        ...
        languageCode: 'ko', regionCode: city?.country_code || 'FR',
      } : {
  ...
  fs.writeFileSync(outPath, JSON.stringify({
    meta: { city_id: cityId, zone, per, pages: maxPages, method, field_mask: STANDARD_TS_FIELD_MASK, called_date: today, dest_count: dests.length },
    zones: zonesOut,
  }, null, 2));
