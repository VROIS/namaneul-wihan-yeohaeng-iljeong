...
if (!cityId) { console.error('Usage: --city-id=<N> [--dry]'); process.exit(1); }
...
(async () => {
  ...
  if (!parsed) { console.error('✗ JSON 파싱 실패 = 잘림 복구도 실패'); process.exit(1); }

  // 7. 카테고리별 응답 수 검증
  const cats = ['heritage', 'hotspot', 'attraction', 'adventure', 'healing', 'shopping'];
  const counts = Object.fromEntries(cats.map(k => [k, parsed.results?.[k]?.length || 0]));
  const total = Object.values(counts).reduce((s: number, n) => s + (n as number), 0);
  console.log(`\n응답 검증:`);
  cats.forEach(k => console.log(`  ${k} = ${counts[k]} / 20 (= 누락 ${20 - (counts[k] as number)})`));
