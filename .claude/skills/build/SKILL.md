...
## ⚠️ 먼저 알 것 — 사장님 배포 흐름에 끼어들지 마라 (2026-08-01 사장님 SSOT)

사장님 흐름은 **이미 완결돼 있다.** AI 가 새 배포 경로를 만들면 곁가지 + 오류만 생긴다
(2026-08-01 실증 = 그렇게 만든 `/deploy` 스킬 폐기 §19).

```
내가 깃 푸시 → 사장님이 Replit pull(코드 전부 따라옴) → 그쪽 AI 점검·서버빌드
            → 웹 · Expo Go · 아이폰12 에서 사장님이 확인
```

= **웹·Expo Go 는 Replit 담당.** EAS 무선 업데이트는 거기에 **안 간다**(다른 창고).

**AI 가 챙길 것 = TF(아이폰)·APK(삼성) 둘뿐**이고, 규칙은 완전히 같다:

| 무엇이 바뀌었나 | TF·APK 반영 | 사장님 할 일 |
|---|---|---|
...
= 이 PC 에 남은 **`android/` 폴더 찌꺼기**(저장소 미추적) 때문이다. 그 폴더를 옆으로 치우고 다시 하면 된다.
**`app.json` 을 고치지 마라** — 2026-07-31 에 그걸 고쳤다가 사장님 앱 설정을 망칠 뻔했다(원복함).
